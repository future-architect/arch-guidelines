...
マルチアカウントの設計は、アカウント分離の単位が最も重要である。環境面ごと（開発・ステージング・本番など）にアカウントを分離することは大前提とした上で、特に、アカウントを跨いで適用すべき施策・一部アカウントのみに適用すべき施策を特定し、各施策に応じた設計が必要である。当然すべてのシステムの要件を満たせる「銀の弾丸」パターンはないが、共通化したい・すべきコンポーネントはある程度パターンが決まっているため、どのような共通化コンポーネントがあるかを整理することで、必要な設計を導き出すことができる。

そのために考慮すべき要件例を以下に示す。

- **各環境へのアクセス権付与**
  - IAMユーザーをどのように集中管理させたいか？
- **監査・GRC（Governance Risk and Compliance）への対応**
  - 内部・外部の監査人へのアクセス権付与や操作ログ・設定変更履歴の保全の仕組みそのものを共通化したいか？
- **セキュリティ監視**
  - 監視対象のログの連携や攻撃証跡保全の仕組みを共通化したいか？
- **既存NWとの接続**
  - 社内NWや開発拠点、DC等から閉域接続やVPN接続が必要か？
  - プライベートIPアドレスの空き状況やコスト面などの要請から接続に必要なリソース(DirectConnectやTransit Gateway)を共用が必要か？
- **外部NWとの接続**
...
- **データ・プライバシーの要件**
  - PCI-DSSなどの基準への適合のため、決済処理などPCI-DSSの対象となるリソース群は『法令・基準適合アカウント』に配置し、それ以外のリソースは通常の『本番アカウント』に配置する必要があるような場合
...
- 集約管理アカウントを設定し、organization内の全アカウントとリージョンのセキュリティ準拠状況を1つのコンソールで管理する。集約先はセキュリティ管理アカウントとする。
...
- ログ保管期間は、監査基準をもとに定める。明確な基準がない場合は、最低1年間の保管を推奨する。ログの保管期間については、「取り扱う情報の機密性や種別、各種監査基準やセキュリティ基準への対応の仕方」を参照して検討する
...
また、セキュリティ基準への対応は、以下の二つの軸で設計する必要がある。
...
| :------------- | :----- | :---------------------------- | :-------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 説明           | ー     | ー                            | 特に暗号方式や鍵長、鍵管理に関してより<br>具体的な推奨事項や要件を示している点が特徴                      | 特に監査頻度、技術要件に関してより具体的な<br>推奨事項や要件を示している点が特徴    | ー                                                                                                                                                                            |
...
- なお、コストの「最適化」ではなく、「最小化」を目指す場合は、1アカウントの中で全てを管理する事がAWSへの支払いとしては最も安価になる可能性が高い
  - 純粋な「最小化」を目指すのであれば、1アカウントで全てを管理する方が総支払額は安価になる可能性がある。ただしその場合、セキュリティや運用のリスク管理、コストの責任分解といった、ガバナンス上の課題を別途解決する必要がある
...
- Gitブランチフロー（参考リンクを参照）におけるブランチ戦略に沿った運用を行う想定で（3）環境に応じて組み合わせる方式を推奨する。
