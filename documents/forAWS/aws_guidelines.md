...
Web APIサーバをどのコンピューティングサービスで稼働させるか、よく利用されるパターンを下表にまとめる。
...
また、Amazon SNSもイベント通知に利用できるが、主に多数のサブスクライバーへのファンアウト型通知を目的としている。メッセージの永続性やコンシューマによる処理管理といったジョブキューの特性や、EventBridgeが提供する高度なイベントルーティング機能とは役割が異なるため、今回の選定からは除外している。
