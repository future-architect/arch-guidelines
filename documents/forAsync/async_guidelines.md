...
またDBにあるデータを登録した後、オブジェクトストレージにもデータを登録するケースなども該当する。どちらかの処理が失敗した場合にデータが不整合な状態で残り続ける（どちらか一方にしかデータが存在しない）ことを許容できない場合は、キューイングを介してデータストアへの書き込みを直列化し、非同期処理側で適切なリトライを行うことで、結果的なデータの整合性を担保できる。
...
コンシューマーが、その処理の一部として、さらに別の非同期処理を呼び出すと、非同期処理が連鎖する。このパターンは、処理のトレーシングや障害発生時の原因特定をさらに複雑化させる要因となり得る。主なユースケースは以下が存在する。
...
非同期処理に利用するキューイングの実行環境は以下の3パターンの方式がある。
...
キューイングにおけるQoS（Quality of Service）とは、以下の3レベルで表現される。なお、すべてのレベルで順序保証がされるわけではない。
...
多くの標準的なキューイング（AWS SQS の標準キューなど）は、スループットを最大化するためにメッセージの処理順序を「ベストエフォート」としており、厳密な順序を保証していない。しかし、非同期処理においてメッセージの処理順序の制御（順序保証）が必要となった場合のその設計方式を検討する必要がある。
...
メッセージロックのタイムアウトとは、キューイングにおいてコンシューマーがメッセージを取得し処理している間、そのメッセージを他のコンシューマーから見えなくし重複処理を防ぐための時間制限のことである。Amazon SQSでは、このメッセージロックのタイムアウトを「Visibility Timeout：可視性タイムアウト」と呼ぶ。
...
| :----------------------- | :--------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------- |
| 図                       | ![アプリ上でDBコミットとキューイングへメッセージ送信](images/direct_dispatch.drawio.png) | ![別プロセスがDBポーリングしてキューイングにメッセージ送信](images/outbox_polling.drawio.png) | ![CDC経由でLambdaを起動しキューイングへメッセージ送信](images/outbox_eventdriven.drawio.png) |
...
（1）の場合は、コンシューマーサービス数（プロセス数）:処理スレッド数=1:1 とすることが基本であるため、流量調整はサービス数のスケールイン/アウトで調整できる。
...
非同期処理におけるセキュリティは、インフラ全般のセキュリティ事項と強く重複する。[AWS設計ガイドライン](https://future-architect.github.io/arch-guidelines/documents/forAWS/) における基本的な対策は実施することを前提とする。また、利用するキューイングは [キューイング環境](#キューイング環境) の推奨にある通り、マネージドサービスを前提とする。もし、その他のミドルウェアを利用する場合は、ユーザー名・パスワードの管理を、AWSであればSystem Manager やKMSなどで機密情報管理をする必要がある。

推奨は以下の通り。

- SQSへのメッセージ登録は、必ずWeb APIサーバーを経由させるなどの構成を取り、直接メッセージングサービスを外部に露出させない
- インフラレベルの基本的なアクセス制御を実施する
  - 最小権限の原則に従い、必要なサービスのみがキューイングへの登録/参照の権限を付与する
...
非同期処理は、キューイングへのメッセージ登録が、事実上API呼び出しとして考えることができる。そのため、キューイングにどのようなペイロードを送信すべきか、インタフェース定義を記載することが望ましい。また、処理が非同期でぶつ切れになるため、機能単位の設計書レベルから全体像を掴むことが難しくなるため、どのようにイベントが伝播するか構成図で捉えられるようにすることが求められる。
