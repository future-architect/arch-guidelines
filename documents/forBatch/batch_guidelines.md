...
| 利用コスト   | ❌️JP1のようにライセンスが必要な場合、調達コストが上がる<br>❌️サーバー利用運用がそれぞれ必要 | ✅️ライセンス数を抑えられる<br>✅️サーバー数を抑えられる                                         |
...
| :------------------- | :-------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------- |
| 分類                 | ワンショット                                                                            | ワンショット                                                                           | 常駐サーバー                                                                                                     |
| 説明                 | バッチ処理用のプロセスを起動させる方式。ここではコンテナアプリを動かすことを想定        | バッチ処理をサーバーレスのファンクションで呼び出す方式。スタンドアローンタスクに近い   | バッチ処理をWeb APIサーバー上で動かす方式                                                                        |
...
| 分離性               | ✅️リソースの独立性は高い                                                                | ✅️リソースの独立性は高い                                                               | ⚠️サーバーリソースを共有している                                                                                 |
...
- バッチ処理の実行時間が分単位（3分以上など）の場合は、（1）を推奨する
  - 実行時間の制限が無いため、初期移行やリカバリなどで定常運用より大きいデータサイズでもタイムアウトせずに実行できる点が良い
...
- 起動時間は、可能であれば業務時間帯に設定すると、運用性が高い
  ...
  - 業務のクリティカル度にもよるが、実行時間の自由度が高い場合、通常の業務時間帯（9-17時など）に起動させたほうが、何かあったときのリカバリ体制を取りやすく、運用者フレンドリーである（※通常、夜間帯は頭が働かないため）
- 起動時間は、バッチウィンドウとリカバリ用のバッファでバランスを取る
  - 例えば、10分程度で終わるジョブを17時までに実行させなければならない場合、16:45起動にすると異常終了した際のリカバリは不可能になる。そのため、門限を遵守する場合は、最低でも16:30、できれば16時に起動しておきたい
...
- ジョブが異常終了した場合に、次回起動を許可するかどうかを、設計で定義する必要がある
  - 起動タイミングで受け持つ対象が決まっているような場合には、異常終了したジョブをリカバリしてから次のジョブを動かす必要があることも考えられる
...
| Cons | ❌️ユーザーの入力情報がバッチ競合で登録不可になる可能性<br>❌️更新内容がバッチで知らないうちに上書きされるおそれがあり、通知やリカバリの有無などの設計や業務調整が必要となる可能性 | ❌️スキップした一覧を表示する画面の準備や、業務運用が必要                                             |
...
バッチ処理のプログラム内でWeb APIを呼び出す場合、並列化したいことがある。ただし、処理レコード単位で並列化すると多重度が高くなりすぎ、依存先のWeb APIサーバーの負荷を高めすぎる懸念がある。そのため、4並列までにするといった絞り込みを検討する。
