...
例えば、下表のような内容はフォーマッタやリンターに寄せられる可能性が高い。
...
「フォーマッタやリンター」などのツールをCIと組み合わせることで、レビュアーが見るべき領域を減らせる。また、「チームで合意された開発規約」をレビュイーが遵守できれば、ツールがカバーできない領域に対してもレビュアーの負担減に繋がる。
...
例えば、Markdownテーブルの項目を編集するとフォーマッタにより大きな差分が出ることも多く、変更に対する心理的ハードルが上がってしまう。しかし、このオプションを利用することでレビュアーの負荷は下がり、本質的なレビューに集中できる。もし、レビュアーがこのオプションを知らないようであれば、 `?w=1` つきのURLをプルリクエストのDescriptionやレビューコメントで追記すると良い。
