...
- **DynamoDBの特性と用途**
  ...
  - ECのカートなど、シンプルなキーで高速な読み書きする要件に向く。性能と拡張性を最大化するために、クエリの柔軟性を犠牲にしているとも言える
- **RDBMSの特性と用途**
...
- 以下のいずれかの要件に該当する場合、それはDynamoDBの採用のノックアウト要件である。そのため、原則、RDBMSを選択または併用を検討する
  1. 将来にわたる全てのアクセスパターンを、設計段階で定義できない場合。ビジネス要件の変更に伴い、未知の検索・集計要件が発生しうるシステムの場合、途方もない開発/運用コスト、インフラ費用がかかり技術的負債となる
...
例えば、過去のプロジェクトでは、適切なインスタンスサイズとクエリチューニングを前提として、秒間700トランザクション（1トランザクションあたり3SQL程度）をAurora（PostgreSQL）で処理できた実績もある。高負荷を理由に単純にDynamoDBを導入するのではなく、まずはRDBMSで対応可能かを見極めるべきである。構成をシンプルにすることで、相当量の設計開発/運用コストを減らせるためである。負荷軽減を目的に部分的なDynamoDB導入を検討する場合は、負荷試験などを通して必要性を慎重に見極めるべきである。
...
- NoSQLとRDBのデータモデルの差異や実装上の課題は多い。特に倉庫在庫棚卸しのような絶対値での在庫数を洗替する処理など、RDB側に加えられた大量のレコード更新をDynamoDB側へ反映させるような双方向同期の要件がある場合は、データ整合性の担保が難しいため適さない
...
[DynamoDB のコストを最小限に抑える | AWS re:Post](https://repost.aws/ja/knowledge-center/dynamodb-minimize-costs)で、属性名を短くするとストレージコストや、RCU/WCU抑制の可能性について触れられている。また、DynamoDBの公式ドキュメントの例示では `snake_case` ではなく `CamelCase` を利用することが多い。
...
DynamoDBのスキーマレスであり、物理的なスキーマ変更自体は柔軟に行うことができる。しかし、誤った設計でホットパーティションやScan利用が必須となり、設計が根本から破綻する可能性がある。そのためRDBMSと同レベルの、緻密な設計が求められる。
...
- 一般的に、（3）のスキーマ設計は（2）のユースケースが分かるまで開始すべきではない（1）までに留めておく必要がある。もし、ユースケースが不明瞭である場合は、DynamoDBだけではなく、RDBMSへの切り替えや併用を検討する
...
（2）のアクセスパターンの洗い出しは以下のように一覧化することを指す（※ユースケースは、ID管理した方が良い）。もちろん、1つのユースケースで複数のテーブルにアクセスすることもありえる。
...
::: tip 隣接リストデザインパターン  
シングルテーブル設計を実現するための代表的なテクニック。1つのテーブル内でエンティティ間の階層関係（1対多）を表現し、関連データをまとめて取得するために使われる。  
:::
...
- 「パーティションキー」と「ソートキー（SK）」の複合キー
  ...
  - 2025/11/19のアップデートで、パーティションとソートキーには最大4つのキーを指定できるようになった。
...
- 3項目以上の複合キーにする場合は、ソートキー側をコンポジットキーにすることを第一に考える
  ...
  - ソートキーは2025年11月以前の、\[属性1\]\#\[属性2\] のように形式で連結せず、マルチ属性ソートキーを利用する。begins_with 演算子を利用して前方一致での柔軟な検索が可能になるようキーの順番は検索要件に合わせて入れ替える
...
2025/11/19 [Amazon DynamoDB now supports multi-attribute composite keys in global secondary indexes](https://aws.amazon.com/jp/about-aws/whats-new/2025/11/amazon-dynamodb-multi-attribute-composite-keys-global-secondary-indexes/) のアップデートで、ハッシュキーやソートキーもそれぞれ4項目まで指定してキーにすることが可能になった。それ以前は `#` などで文字列連結したキーを独自に作成する必要があった。ほとんどのケースで4つあればハッシュキー、ソートキーとしては充足すると考えられるため、従来の文字列結合による独自キー生成はストレージ容量面で不利になるため、不要になると考えられる。
...
GSIのプライマリーキー（PK/SK）に設定する属性には、Null型や空のバイナリ/文字列を設定できない（実行時に、ValidationExceptionエラーになる）。スパースインデックスを実現したい場合は、Nullをセットするのではなく、アイテムからその属性自体を含めない（削除する）必要がある。  
:::
...
複数の属性値を特定の区切り文字（例: \#）で連結し、単一の文字列としてPKやSKに格納する設計パターンは、DynamoDBでは複合キーやコンポジットキーと呼ばれる。RDBMSではまずお目にかかれないが、2025/11/19のアップデートまでは、DynamoDB界隈では一般的であった。それ以降はマルチ属性キーを用いる。PKは完全一致が必要であるため、SK側をマルチ属性キーにし、範囲検索に対応したり、1つのインデックスで複数の属性に対する絞り込みやソートを表現し、GSIの数を減らすことができる。GSIのプライマリーキーのマルチ属性キーは2025/11/19時点で最大8つサポートされる。
...
KEYS_ONLYからINCLUDEのように、射影タイプ自体を変更できない。INCLUDEでは、作成済みのGSIに対してINCLUDEに含める属性を後から追加・削除ができる。
...
::: tip 一律ALLにするという選択肢  
KEY_ONLYやINCLUDEの射影にした場合、RDBMSとのメンタルモデルと乖離し開発者が混乱することも多い。例えば、DynamoDBで属性追加になった場合、GSI経由で取得する場合はその属性がINCLUDEの射影対象に入っていないため、取得不可といった事象にハマることが多い。そのため、費用を犠牲にしてでも一律ALLで倒すという設計判断もありえる。実際、複数のプロジェクトでALLに寄せた設計を採用した実績がある。  
なお、システム共通項目だけ取り除き、残りの全属性をINCLUDE対象にするという方針も考えられる。しかし、これは項目追加時にINCLUDE側の属性に追加することが忘れやすいというデメリットがありオススメしない。INCLUDE設定で、明示的に項目除外設定ができれば嬉しいが、そういったオプションは存在しない。  
:::
...
DynamoDBの場合も、原則、[共通カラム | テーブル論理設計 | PostgreSQL設計ガイドライン](https://future-architect.github.io/arch-guidelines/documents/forDB/postgresql_guidelines.html#%E5%85%B1%E9%80%9A%E3%82%AB%E3%83%A9%E3%83%A0) に従う。ただし、patched_at, patched_by, patched_id は、データパッチ運用時に利用するものであり、アプリケーション設計からは除外する。DynamoDBはスキーマレスであるため、後から属性を追加できる。

また、TTLを利用した改廃する場合は、最初から `ttl` 属性を付与しておくこと。
...
- `UpdateItem`で可能な更新処理の場合は、`UpdateItem`の利用を第一に考える
  - インクリメント/デクリメントなどのアトミックカウンターや、既存の属性値に基づいて更新する際は、競合条件の防止とパフォーマンス向上のため`GetItem`と`PutItem`を組み合わせた「Read-Modify-Write」サイクルではなく、`UpdateItem`の`ADD`や`SET`アクションを利用する
...
DynamoDBの書き込み操作（PutItem, UpdateItem, DeleteItem）では、ConditionExpressionを使用して、特定の条件が満たされた場合にのみ操作する、データ一貫性を保つ仕組みがある。主なユースケースは以下がある。
...
- （2）と（3）を組み合わせる
  ...
  - 数回の再試行でも成功しなかった、本当に処理不能なレコードのみDLQに送られるため、メインのストリーム処理へのブロッキングがなくなり、失敗したイベントは後から調査＆リカバリが可能
...
DynamoDBからDWHやデータレイクといったOLAP系のシステムへデータ連携が必要な場合、主要な方式には以下がある。
...
- ユニットテストでは、テスト効率の観点から本物のDynamoDBへ接続する代わりに、原則、ローカルモックを使用する
...
- 原則、PITRは全てのマスタ・トラン系のテーブルで有効にする
  ...
  - 費用などの観点で、別経路でリストア可能なバックアップが取れているのであれば、不要にしても良い（例えば、DymamoDB Streams経路でS3などにバックアップが時系列で蓄積されているなど）
- 期間は最大値（35日）を一律設定とする
...
- **構成シングルリージョン**: DynamoDBは自動的に複数AZにデータを複製し、AZ障害への耐性がある。リージョン全体の障害で数分～数時間のデータ損失が発生する可能性
...
- もし、明確な復旧計画を立てるようなプロダクト特性やプロダクトフェーズでなければ、（1）から初めて成長に応じて（2）に移行する
