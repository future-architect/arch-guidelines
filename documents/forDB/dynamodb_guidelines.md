...
# 例2: 本番環境のfugaプロダクトにおけるユーザーアクセスログテーブル
...
- `UpdateItem`はアイテムが存在しなかった場合に意図しないレコード作成の懸念があるため、`attribute_exists(PK)`を利用する
...
| Lambdaバッチサイズ | ⚠️アイテムの平均サイズによってはLambdaのバッチサイズの上限6MBに引っかかりやすい。例えば、平均200KBの場合、新旧で400KBになるため、15レコードほどになる | ✅️最適化したスループットを出しやすい                                                                                    |
...
- 特別なコンプライアンス要件をまず確認する
  - 例えば、PCI DSS対応する場合、カスタマー管理キーにする必要がある
