...
VS Code上でのGit操作は、サイドバーの "Source Control" から行える。ほとんど全ての操作はコマンドパレットからも実行可能だが、説明は割愛する。
...
なお、フェッチ後に以下のようなダイアログが表示された場合、 "Yes" を選択すると自動で定期的にフェッチを行う。
...
- (a) SOURCE CONTROL パネル > 三点リーダーアイコン (More Actions...) をクリックし、Branch > Create Branch... を選択
  - 現在チェックアウトしているブランチから新規ブランチが作成されるが、Create Branch From... を選択すると、作成元のブランチを選択できる
