...
ドメイン認証にはSPF(エスピーエフ)・DKIM(ディーキム)・DMARC(ディーマーク)の3つが存在し、いずれもDNSを活用するが、それぞれ異なる仕組みでメールの信頼性向上に寄与する。
...
| t    | DKIM認証時の挙動を指定するフラグ（2025年7月現在`y`と`s`のみ有効）<br> `y`: テストモードであり、署名の検証に失敗した場合も、未署名のメールと同等の扱いを要求する。<br> `s`: 送信元メールアドレスのドメインと、署名を行ったドメインが一致しない（サブドメインが用いられている）場合に認証を失敗させる。<br> なお、`y`と`s`はコロン”:”で区切ることで両方指定することが可能。 | 任意 省略時はフラグ未指定となる。                                                                                     |
...
この場合、Gmailなど一部のメールクライアントでは「[amazonses.com](http://amazonses.com) 経由」のような表示がなされたり、場合によっては迷惑メールとして扱われる・受信拒否されてしまうなど、メール配送に影響が生じる。

対策として、ほとんどのメール送信サービスでは、サービス側が提示するレコードをDNSに登録することで、独自ドメインを用いてドメイン認証が行える（=DMARCアライメントが成功する）仕組みが提供されている。
...
| 日本の携帯キャリア     | ❌非対応   | \+記号は、アドレスの一部として有効な文字として扱われるため、+エイリアスをつけると完全に別アドレスと見なされる                                                                                        |
...
| :------------------ | :----------------------------------------------------------- | :----------------------------------------------------------------------------- |
| 説明                | 海外の通信網を経由して、日本の携帯キャリア網に接続される経路 | 日本のSMS送信事業者が、国内の携帯キャリアの設備と直接接続してSMSを配信する経路 |
