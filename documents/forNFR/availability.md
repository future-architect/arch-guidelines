...
稼働率とは、期待（運用スケジュール）に対して実際にどれだけサービスを提供できたかの割合であり、値が高いほど期待に近い。ここで注意したいのは、停止時間には障害そのものの継続時間だけでなく、その後の復旧処理の時間も含む点である。
...
MTBF（平均故障間隔）や故障率といった伝統的な信頼性の指標は、本章では独立して扱わず、継続性（稼働率・RTO／RPO）に含めている。利用者が最終的に気にするのは、部品がどれくらいの頻度で壊れるかではなく、結果としてどれだけ止まらなかったか（稼働率）、壊れてもどれだけ早く戻せるか（RTO・RPO）だからである。稼働率は、故障間隔（MTBF）と復旧時間（MTTR）を合わせた結果指標であり、そちらで充足する。
...
- **クラウド**  
  公開値は、設計値というよりベンダーの財務上の防衛ライン（商用上の下限）に近い。実際の設計値はより高いはずだが開示されないため、鵜呑みにはせず「最悪でもこの水準」とみなす。その数字だと毎月どのくらい停止するかをタイムラインで描いて業務の期待に足りるかを確かめ、運用が始まったら自分たちの実績値で見直していく。
