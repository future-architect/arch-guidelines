...
- **主要機能要件**  
  ...
  - **例2: リアルタイムな通知:** 都度リクエストを投げるHTTPでは要件を満たせず、WebSocket などの常時接続が必要となると話が大きくなる。各接続を保持するステートフルな構成や、スケールアウト時のセッション共有やメッセージ配信のためのPub/Sub基盤、WebSocketに対応したロードバランサまで含めて構成が必要となる。
- **制約**  
...
後者に引きずられて「基本設計で決めればよい」と先送りすると、基本設計で「その要件にはインフラの抜本的な変更が必要」となり、予算超過やスケジュール遅延を招く。だからこそ、一度に決めきるのではなく段階的に、かつ早期に、大枠から見切っていく必要がある。
...
もし、該当者が不明の場合（例: 運用保守の引き継ぎ先が未定）は、構築側が仮説の上で想定を代弁し、後日引き継ぐ前提で進める。
...
- **過剰品質を避ける**
  - 非機能要件は不足していることばかりが問題視されるが、過剰であることも同じくらい大きな問題を引き起こす。念のため可用性を高くする、念のためデータを暗号化するなど、念のための積み重ねは複雑度を上げて開発コスト・運用保守コストが嵩み、リリースまでのスピードを犠牲にする。常に要件の根拠を問い、明確な根拠がない場合は過剰である可能性を疑い、必要十分を見極める
- **重要な要件を見つける**
...
重要ドライバを決めるために有効な問いは、「このシステムのコンセプトは何か？」である。ビジネスコンテキスト（[STEP2](#step2)）を踏まえてこの問いに答えることで、各要件を仮説として立てる際の "どちらへ寄せるか" の指針になる。
