...
**本ガイドラインで扱う文書の前提**
次のように多岐を想定している。その文章が求められているコンテキストごとに求められている期待や文量が異なるが、なるべく一般的に適用できる事項から紹介している。
...
1. **設計**: 書き出す前に読み手との期待値を合わせ、伝えるべきメッセージと文脈を定義する
2. **構造化:** 複雑な情報を直感的に伝えるため、箇条書きや表の使い分け、論理的な構造に応じた図解パターンを活用する
3. **簡潔さ:** 相手の時間を奪わず正確に伝達するため、一文一義の徹底や冗長な表現の削減など、情報密度を最大化させる
4. **表現の正確性:** 読み手の認知負荷を下げ、誤解を防ぐための正しい用語選択や、認識を揃えるための具体例の適切な使い方を扱う
5. **実践応用:** 実務ですぐに使える型や、アンチパターンの紹介
6. **セルフレビュー: 自身での推敲**
7. **レビュー:** 他者の文章をレビューし、チーム全体の書く力を育てるためのフィードバック技術
...
flowchart LR
    ...
    subgraph sg2 [執筆]
        direction TB
        step2["2. 構造化"]
        step3["3. 簡潔さ<br>（情報密度の最大化）"]
        step4["4. 表現の正確性"]
        step5["5. 実践応用<br>（型・アンチパターン）"]
...
3. **相手が意思決定に必要な「判断材料」が充足しているか？（What/Why）**
   - 「AとBどちらが良いですか？」「IAMロールを追加してよいですか？」とだけ聞かれても、相手は “知らんがな” となり判断できない。それによる影響度が何で、リスク有無、発生費用（時間）がどれくらいで代替案はあるのかなど、相手が意思決定を下すために必要な比較材料や前提情報を提示する責任は、書き手にある
...
ソースコードを読めば分かるWhat（何を作ったか）や、チケットを見れば分かる「やったこと」を日本語に翻訳するだけでは、書き手が介在する意味が薄い。ドキュメントの目的に立ち返り、読み手が真に必要としている情報に焦点を当てる必要がある。

- **目的が「設計の妥当性評価や将来の保守」である場合**
  - 例: 設計書など。システムの仕様をブラックボックス化しないことが目的で説明することは多い
...
この話をすると高確率で「工数が..」という話になる。真実、工数が無く形骸化が是正できないのであれば、思い切ってそのルールをもっと簡略化して思い切って書かないとか、業務を断捨離化する方向に倒すのも取り得ない案ではない。あなたがリーダーであればそういった意思決定を主導してみると良い。メンバーの場合は1on1などでそれとなく相談してみると良い。
...
執筆者と読み手の間に存在する情報の非対称性を埋めることで、文章の唐突さや難解さを軽減し、読み手に「文章を読む準備」を促すことができる。

::: info 例: 唐突なアクション要請

- ❌ **NG例**  
  〇〇の設定を至急変更してください。
- ✅ **OK例**（ラベルは説明のため便宜的に付与している）  
...
顧客のキーマンや上長は、日々圧倒的な情報量を処理している。そのため、自分が強い関心を持っている事案以外、「前回までに何を話し合い、どう決着したか」を詳細には覚えていないことが多い。 相手が記憶している前提で進めると、認識のズレや「そもそもこれって何の話だっけ？」と確認のラリーが発生し、特定に時間を要する。
...
本番環境でのシステム障害、どうしても解決できないバグの相談など、相手に状況の把握と急ぎの支援を求める時は、STAR（Situation（状況）→ Task（課題）→ Action（行動）→ Result（結果））が便利である。障害時に「DBをトランケートさせてください」と結論だけ伝えると、相手は「ちょっと待って、そもそも何が起きていて、影響範囲は何だっけ？」となるだろう。この場合は何が起きているか状況を同期する必要がある。
...
コンサルタントやアーキテクトが扱う課題は、往々にして複雑に絡み合っている。情報を分類・ラベル付けして抽象化することで、問題の全体像ははるかに捉えやすくなる。 読み手であるクライアントや上長の関心事は、細かな実行レベルの詳細よりも、「問題の構造」や「解決の方向性」を大まかに、しかし正しく把握し、大枠をチェックすることにある場合も多い。そのため、複雑な情報を構造化すること自体が我々の付加価値であると認識すると良い。
...
リストや表の直前には、必ず内容を説明する一文を置く。読み手がこれから提示される情報が、前提条件なのか作業手順なのかと言った役割を認識した上で、読み始めることができるため。

::: info ✅️ OK例：1行目の導入文により、リストの役割を明確にする
...
::: info ❌ NG例：指示語や言葉足らずで、内容が不明瞭

- これを適用する
- 前述の構成図を参照する
  :::

::: info ✅ OK例：項目単体で意味が通じる
...
::: info ✅ OK例：情報の役割ごとにラベルを付け、関連する文をまとめている
...
::: info ❌ NG例（中身を開かないと目的がわからない

- 〇〇画面について
- 〇〇画面のダウンロードボタンについて確認
  - 理由：画面の何を確認したいのか、アクションが不明
    :::

::: info ✅ OK例（画面名・対象・アクションがすべて含まれている）
...
- ❌ **NG例**  
  ...
  ※理由: 最後まで読まないと「確認」なのか「提案」なのか、相手に求めているアクションが不明なため
- ✅ **OK例**  
...
「いつ」という問いに対して、状況説明や言い訳を返すと、相手は「で、結局いつなの？」と再度質問（ラリー）をしなければならなくなる。

- ❌ **NG例**  
  現在、〇〇チームに確認をしているところで、まだ返事がきていないです。後ほどリマインドを入れておきます。  
  ※理由: 状況の説明に終始しており、「いつ」という問いへの直接的な回答が欠けている
- ✅ **OK例**  
...
- ❌ **NG例**  
  ...
  ※理由: 一文に多くの情報が入りすぎている。「～については」のような主題提示は単独の文に切ることが望ましい
- ✅ **OK例**  
...
※理由：複数の情報が1つの文章に埋もれてしまい、修正箇所の数や内容を瞬時に把握しにくいため。
...
- ❌ **NG例**  
  ...
  ※理由：接続詞が多く、完了報告なのか注意喚起なのかが伝わりにくい。
...
- ❌ **NG例**  
  ...
  ライブラリAが一般に広く使われています。
  ※理由: この「～が」は読み手が続きを予測しにくいため、避けたほうが良い
- ✅ **OK例**  
...
名詞中心の文章や、主語が人間や生き物ではない文章は、特に生成AIの出力に含まれがちである（英語の学習データの影響であろう）。これらはフォーマルな印象を与えるという効果を持つ一方で、だれが何をするのか？という直感的な理解を妨げる。そのため、動詞を中心とした自然な日本語への書き換えを行うべきである。

::: info 例：AI特有の「重たい名詞句」 解消

- ❌ **NG例**（名詞の使いすぎ）  
  GPT-5のモード切り替えシステムにより、ユーザーは手動でモデルを選択する手間から解放され、常にコスト効率と精度の最適なバランスを享受できるようになった 。
  ※理由: 「手動でモデルを選択する手間」「コスト効率と精度の最適なバランス」などの長めの名詞句が多用されており、把握しづらい
- ✅ **OK例**  
...
- ❌ **NG例**（無生物主語になっている）  
  ...
  ※理由: 「『Extended Thinking』モードは……能力を提供する」という無生物主語になっている。また、目的語が長い名詞句になっている。
- ✅ **OK例**  
...
- ❌️ **NG例**: IT業界の標準用語である「データスチュワード」
- ✅️ **OK例**: 顧客社内で浸透している「データ管理者」
  :::
...
- ❌ **NG例**: 管理画面のAPIは、ユーザーのロールを"認証"してから処理を実行する。
- ✅ **OK例**: 管理画面のAPIは、ユーザーのロールを"認可"してから処理を実行する。
  :::

2. 一貫性  
   文中で同じ呼び方を維持できているか。また、1つの概念に対し、1つの用語が1対1で対応しているか

::: info ❌**NG例**: ユーザー、アカウント、お客様の揺れ。読み手は何か意図があるのか、余計な推測を強いられる
...
2. **略称の利用是非**
   ...
   - **推奨**：元の単語より略称になることで劇的に短くなる場合、かつ文書中の出現回数が多い時のみ略称の利用を検討する
3. **用語集を活用する**
...
- ❌ **NG例**（対象範囲が不明瞭）  
  ...
  ※理由: DBのデータは戻るが、外部APIによる決済処理や送信済みのメールはどう扱うのかが不明瞭である
- ✅ **OK例**（システム境界を明確にする）  
...
- ✅ OK例: 「本計画書における「性能」はオンラインレスポンス時間を指し、夜間バッチの処理時間は含まない。」
  :::
...
**▼期待するレベル感をすり合わせる**  
定性的な判断基準は属人的になりがちだが、具体的な事象を添えることで読み手とレベル感を揃えられる。
...
**▼求めるアウトプットの方向性と粒度を示す**  
漠然とした要求でも、例を1つ提示するだけで求めるアウトプットの方向性が明確になる。
...
読み手と既に共通認識が取れている概念への例示は、本質的に伝えたいことを埋没させてしまうため避ける。

::: info ❌ NG例
機密情報（例：顧客の氏名、住所、電話番号、生年月日など）をローカルPCに保存しないこと。  
※理由: プロジェクト内で「機密情報」の定義が合意されている場合、一部の項目だけを例示すると「例にないメールアドレスは保存して良いのか？」といった不要な推測を生むリスクがある。
:::
...
- **箇条書き**  
  ...
  情報の受け手からしても、散文で書かれているよりも視認性が高まり、要素の数が一目で分かるメリットがある。
  スライドにおいては、デフォルト機能の箇条書きを使うよりも、種々のオブジェクトやアイコンを配置することでより視認性を上げる事ができる。箇条書きした結果、対象が同質である場合は対比・マトリクスに変化させることで、より論理構造を明確化させることもできる。
- **対比（変化）**  
...
- **ツリー**  
  問題の分解や組織構造を図示する場合はツリー型を用いる事もできる。  
  ツリー型は上位概念から下位概念への流れるような論理性を提供することができ、所謂MECEであることの説明にも有用である。スライド上では、左から右、あるいは上から下へと階層が深まるように配置し、視覚的な一貫性を保つことが求められる。
...
例えば報告書や設計書など、非機能グレード表など、期待されるテンプレートがあるのであればそれを活用して時短で構造を決めて、中身の方に時間を割くようにする。ただし、官僚的にならないように不要な項目は省くなど過不足なく情報が伝えられるように心がける。

型をどれだけ知り、どれだけ引き出しに入れておけるかが、質の高い文章を効率よく生産するうえでのポイントとなる。毎回、ロジカル・シンキングの原理原則からボトムアップで作成するのは労力がかかりすぎる。また、型がないと、AIに文章を作らせるままでアウトプットの品質が安定しない、ということにもなりかねない。

### 最小限のメッセージの型を覚える

業務上の「他者に行動を促す」か「自分の行動の許可を得る」メッセージを送信する場合、メッセージの型を覚えておくと便利である。

::: info `[型]  = (依頼事項や確認事項）＋ その意図`

- ✅️ **OK例**: 行動を促す例  
  @member 機能Aを参考に、機能Bの実装を進めてください。来週から機能Cの実装を進めたいため。

- ✅️ **OK例**: 許可を得る  
  @leader 明日の13時から、打合せの時間を入れさせてください。タスクAの進め方について確認したいです。
  :::

他にも、以下のように5W1Hなどを参考に、追加したパターンも引き出しに持っておくと良い。

- `[型]  = （依頼事項）+ 意図 + 期日 + エスカレーション条件（XXが無理そうならXXまでにコメント下さい）`

# **アンチパターン**

一見問題がないように見えても、ユーザ目線でのユーザビリティを下げたり、長期的なドキュメントの品質を損なうようなアンチパターンが存在する。

## ユーザビリティの低下

テクニカルライティングではドキュメントの内容がユーザにとって利用しやすいことが重視される。内容の理解を直接妨げない場合でも、コマンドの再利用や手順の実行において不便を生じさせることがある。ドキュメントを実用的にするという観点で避けることを推奨する。

### テキストのスクリーンショット

テクニカルライティングにおいてはコマンドプロンプトやターミナルなどのCLIの動作を取り扱うことも少なくない。その中でコマンドなどをテキストとして載せるのではなく、CLIのウィンドウなどをスクリーンショットとして載せてしまう人がいる。これは避けるべきである。
...
一方でテキストであってもフォントのデザインの確認など、テキストであっても見た目に対して言及が必要な場合についてはこの限りではない。

### 指示語のリンクを使用する

「ここ」や「こちら」などの指示語にハイパーリンクを設定する場合があるが、避けるべきである。

理由:

- ユーザーはリンク先の内容がわからず、可読性が低下する
- ページ内検索の検索対象としにくくなり、利便性が低下する
...
```shell
gitでリポジトリをクローンする
$git clone https://exmaple.com/hoge.git...Cloning into 'hoge'...
```
````
...
````text
```shell
git clone https://exmaple.com/hoge.git...
```
````
...
手順などを記載する際にGUI上の操作の記載が必要になる場合もある。その際に「右上」や「左側」などの位置だけで説明をすると、UI変更や画面サイズの違いなどによって説明が成立しなくなる場合がある。そのため、GUIの操作を手順で説明する場合には可能な限りボタン名やメニュー名などの、UI上の位置と比べて不変的な説明を併記することが望ましい。
...
::: info 例: 「アクセス権限制御のために新しくユーザーグループが必要」ということを伝えたい場合のメッセージ文
メッセージ分は、「アクセス権限について相談」ではなく「◯◯画面のアクセス制御について、顧客のAさんに、管理用のユーザーグループを新規に払い出してほしい」がメッセージ文になる。

- **❌ NG例 :** アクセス権限についての相談
- **❌ NG例:** 新規払い出しの依頼
- **✅ OK例:** ◯◯画面のアクセス制御について、顧客のAさんに、管理用のユーザーグループを新規に払い出してほしい。
...
- **メッセージ文（目的）とのズレを指摘する**
  - **❌ NG例 :** 「この文章、よくわからないよ」
  - **✅ OK例:** 「この文章は、◯◯と読めるけど、それってメッセージ文で狙っていることと合っている？」
- **ガイドライン（作法）とのズレを指摘する**
  - **❌ NG例:** 「ガイドラインのとおり書いて」
  - **✅ OK例:** 「この文章の◯◯の部分は、ガイドラインの◯◯という部分に沿ってないから直してね」
...
- ❌️**NG例**: 「わかりにくい」「ガイドライン読んでから書いて」
  - 読み手がどう映るかの情報が含まれないため、改善のヒントが含まれない
- ❌️**NG例**:「わかりやすかった」「スッと入ってきた」
