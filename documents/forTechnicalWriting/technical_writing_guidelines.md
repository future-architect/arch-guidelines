...
「なぜ他ではなく、これを選んだのか」という判断のロジックを、客観的なトレードオフを用いて説明可能にすることが重要である。
...
障害対応の記録や課題管理票では、必ず「誰が（Who）」「いつまでに（When）」「何をするか（What）」というネクストアクションを明記する。具体的なアクションを定義することで、読み手の次の行動を誘発する文書となる。
