...
本ガイドラインは、専門概念を構造化し、判断に足る情報を示して意思決定を推進するためのライティング技術をまとめる。
...
- **1次元のテキストを目次で構造化する**  
  文章は上から下へ1次元で読まれるので、情報の順序が理解度を左右する。チャットなどの短文を除き、まずは目次を作成し、情報を伝える順番を設計してから本論の執筆に入る
- **単独で読める資料を前提とする**  
...
実務の報告において、事実や推測（空・雨）だけで止まってしまうケースが多い。「〇〇について調査したところ、▲▲に課題がありました」という報告だけでは、読み手に「で、自分はどうすべきか？」をゼロから考えなければならない。アクションの立案を相手に丸投げすることは、書き手の介在する付加価値が低いことを意味する。相手が知りたいのは、「あなたがどうしたいのか（そのための承認をもらいたいのか）」あるいは「私は何をすればよいのか」という「傘」の部分である。

::: tip 「傘」が欠けていないかのチェック  
自分が伝えようとしているメッセージが単なる事実の羅列になっていないかを確認するために、前項の5W1Hがそのまま使える。「誰に」「何をしてほしいか」の要件を満たそうとすれば、自然と傘（見解）が必要になるからである。  
:::
...
この話をすると高確率で「工数が..」という話になる。真実、工数が無く形骸化を是正できないのであれば、思い切ってそのルールを簡略化するとか、業務を断捨離化する方向に倒すという選択も取り得る。あなたがリーダーであればそういった意思決定を主導してみると良い。メンバーの場合は1on1などでそれとなく相談してみると良い。
...
数日ぶりに再開するSlackのスレッドや、長期間オープンになっている課題チケットの更新、あるいは定期的な報告書を作成する際は、相手が完全に文脈を忘れているという前提で冒頭に補足する。
...
（理由）理由は、直近のユーザーアンケートで操作性への不満が多く挙がっているからである。  
...
（状況）本番環境にデプロイした後、一部ユーザーでログインできない事象が発生している。  
...
情報を構造化する際は、要素間の関係性に応じて適切なフォーマットを選択する。 その際、情報の整理や関係性の明確化に優れた上位のフォーマット（表 ＞ ラベル付きリスト ＞ 番号付きリスト ＞ 箇条書き）の順に適用できないかを検討することが望ましい。ただし、要素間に明確な比較軸や順序がないにもかかわらず、無意味に上位の構造を適用する必要はない。
...
箇条書きや見出しを使って情報を整理した際、「設計、開発、CI/CD」のように抽象度の違うものが同列に並んでいると、読む気を失いアウトプットとしては論外である。レベル感を揃え、漏れやダブりが無い状態、つまりMECEを保つ必要がある。粒度の細かい要素が混ざっている場合は、上位の概念でネストさせる。
...
::: tip 特にMECEさを求められるドキュメント

機能要件を定義するドキュメントでは、後続フェーズでの手戻りや実装のブレを防ぐため、要件がMECEな構造になっているかに留意する。
正常系（ハッピーパス）だけでなく、業務とシステムの両面から、異常系や境界値（エッジケース）における振る舞いが網羅的に定義されている状態を目指す。

:::

### 導入文を置く

リストや表の直前には、必ず内容を説明する一文を置く。読み手は、これから提示される情報が前提条件なのか作業手順なのかといった役割を認識した上で、読み始めることができるからである。
...
一連の文章を単に短く切り離して並べると、論理関係（因果関係や接続詞の意図）がなくなり、かえって理解しづらくなる。各項目がどのような意味を持っているのかを表すラベルを付けることで、情報の構造を明確にする。
...
- ✅ **OK** （情報の役割ごとにラベルを付け、関連する文をまとめている）
  - 【依頼】システムを再起動する
  - 【補足】設定が反映されない場合があるので、30秒待機してから再度試みる
...
メール、GitLab、Redmineなど、件名/表題については内容を具体化する。中身を見なくても判断できるようにする。多忙なキーマンは通知一覧の件名だけを見て対応の優先順位を決めているので、「〜について」といった曖昧な表現は避ける。
...
件名がない場合でも、一文目で目的（確認・相談・報告）を明示する。読み手が抱く、何の話をどういったスタンスで読めば良いのか？ という疑問を最初に解消する。
...
- ✅ **OK**  
  【相談】〇〇機能の◇◇の仕様について相談したいです。現状XXとなっています。しかし、△△のように操作をすると問題が発生するので、◇◇にしたいと考えます。ご意見をお聞かせください。
  :::
...
不必要な要素や繰り返しは、削除すれば簡潔になる。それらが無い場合は、複数の文に分割する、箇条書きなどの構造化を試みる。
...
- ✅ **OK**  
  ...
  ただし、一部データが古いので、確認の際はご注意ください。

:::

### 「～が」を逆接以外で使わない

「～が」は、逆接にも使えるが、単純に文をつなげる際にも使える。逆接以外のつなぎ方で用いると、読み手が要素間の関係を把握しにくくなるので、避けたほうが良い。
...
### 理由は「から」「ので」で示し、「ため」は目的に絞る

「ため」は目的（〜するために）と理由（〜だから）のどちらも表す。1つの語が2つの役割を兼ねると、読み手はどちらの意味かを文脈から読み取る手間を負う。気を抜くと1つの文章の中に両方の「ため」が混ざるので、理由を示すときは「から」「ので」を使い、「ため」は目的に絞る。理由が長くなるときは前述の一文一義を優先し、文を分けて「〜からである」と独立させる。

::: info 例: 同じ「ため」が目的と理由の両方に使われている

- ❌ **NG**（1文目は目的、2文目は理由で、同じ語が別の意味を持っている）  
  リリース日を守るため、機能Bは次回に回します。  
  テスト環境の準備が間に合っていないためです。
- ✅ **OK**  
  リリース日を守るため、機能Bは次回に回します。  
  テスト環境の準備が間に合っていないからです。
...
2. **略称の利用是非**
   - 略称の濫用は正確性を損ね、読者の認知負荷を高める可能性がある
...
2. **未対応の作業リスト（単なるToDoリスト）**
   - ⚠️ チーム内での呼び方であればよいが正確な用法でないので非推奨
...
- ❌ **NG**（メールアドレスの記載がないのはなぜか？ といった疑問を生むリスクがある）  
...
特殊な業務ルールなど、具体例の理解に「説明のための説明」を要する場合、散文での例示はかえってノイズになるので避ける。複雑な条件分岐を含む場合は、無理に文章で説明せず決定表や状態遷移図等の「構造化」を検討する。
...
一見問題がないように見えても、ユーザー目線でのユーザビリティを下げたり、ドキュメントの長期的な品質を損なうようなアンチパターンが存在する。

## ユーザビリティの低下

テクニカルライティングではドキュメントがユーザーにとって利用しやすいことを重視する。内容の理解を直接妨げない場合でも、コマンドの再利用や手順の実行において、ユーザーに不便を強いることがある。ドキュメントを実用的にするという観点で避けることを推奨する。
...
コマンド実行などをコードブロックとして扱う場合に、下記のような不要な情報をコードブロックに残すことは避けるべきである。
...
- どの部分を参考にさせるかが読者次第となり、読者への負荷を引き上げる
...
手順などを記載する際にGUI上の操作の記載が必要になる場合もある。その際に「右上」や「左側」などの位置だけで説明をすると、UI変更や画面サイズの違いなどによって説明が成立しなくなることもある。そのため、可能な限りボタン名やメニュー名などの、UI上の位置と比べて不変的な説明を併記することが望ましい。
...
「〇〇の構成案を作成しました。現在は構成と結論の方向性を確認したいフェーズです。細かい文言の修正は後ほど行うので、まずはロジックに破綻がないかを中心にレビューをお願いします」  
:::
