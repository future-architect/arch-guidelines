...
3. **実践応用**
   - 実務ですぐに使える型（パターン）や、アンチパターンの紹介する
4. **品質向上**
   - セルフレビューやレビュー依頼の方法を抑える
5. **育成**
...
::: info 例：記録文書におけるネクストアクションの明確化

障害対応の記録や課題管理票では、必ず「誰が（Who）」「いつまでに（When）」「何をするか（What）」というネクストアクションを明記するとよい。
具体的なアクションを定義することで、読み手の次の行動を誘発する文書となる。

:::

## トップダウンアプローチとフラクタル構造で整理する

抽象度の高い全体コンセプトから、徐々に詳細へとドリルダウンする構成を取ることで、読み手の認知負荷を下げることができる。
特に、コンサルタントやアーキテクトの考え（Why/What）を提示し、ステークホルダーと方針の合意を得るための文書（提案書、アーキテクチャ方針書など）において有効である。

長編ドキュメントでは、以下のようなフラクタル構造を意識し、階層ごとにメッセージ（主張）を配置すると論理的な一貫性が保たれる。

- **全体**: 目的 →　概要 →　詳細 →　結論
- **章・節**: 概要 →　詳細 →　まとめ
- **項・段落**: トピックセンテンス（主張の要約文）→ 説明 →　補足

```mermaid
graph LR
    subgraph sg3 [3. 項・段落]
        direction LR
        step8["トピックセンテンス<br>（主張の要約文）"]
        step9["説明"]
        step10["補足"]
        step8 --> step9 --> step10
    end
...
::: tip MECEさを求められるドキュメント

機能要件を定義するドキュメントでは、後続フェーズでの手戻りや実装のブレを防ぐため、要件がMECE（漏れなく、ダブりなく）な構造になっているかに留意する。
正常系（ハッピーパス）だけでなく、業務とシステムの両面から、異常系・境界値・エッジケースにおける振る舞いが網羅的に定義されている状態を目指す。
...
名詞中心の文章や、主語が人間や生き物ではない文章は、特に生成AIの出力に含まれがちである（英語の学習データの影響であろう）。これらはフォーマルな印象を与えるという効果を持つ一方で、だれが何をするのか？ という直感的な理解を妨げる。そのため、動詞を中心とした自然な日本語への書き換えるべきである。
...
::: tip 読み手に推測させてはいけないドキュメント

要件定義書やシステム設計書など、システム要件（What/How）を定義するドキュメントにおいて、読み手のスキルレベルや業務知識に依存した「行間を読む」ことを要求してはならない。
解釈のブレが生じやすい多義的な用語は冒頭で定義し、誰が読んでも解釈が一致する表現を徹底する。
...
プロジェクトによっては、推奨事項として `WANT` や `RECOMMENDED` を使うケースもあるが要件レベルの表現についてはチーム内で一貫性を保つように
...
:::tip RFC 2199 で定義されている要件レベル

インターネット技術の標準仕様であるRFCでは以下のキーワードで要件レベルを表すように規定されている。

> The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in RFC 2119.

参考: [RFC 2199: Key words for use in RFCs to Indicate Requirement Levels
...
- ✅ OK（背景と不採用案の記録）  
  ...
  **背景・制約**: 運用チームの学習コストを抑えるため、自前でのサーバ管理が不要なフルマネージドサービスであることを最優先とした。  
  **不採用案**: OSSの「Apache Airflow」も有力な候補であったが、自前でのサーバ構築や定期的なバージョンアップの手間（運用コスト）が、現在のインフラ運用チームの要員体制では許容できないと判断し、今回は不採用とした。
