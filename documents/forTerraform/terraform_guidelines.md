...
[JSON Configuration Syntax](https://developer.hashicorp.com/terraform/language/syntax/json) にあるように、Terraform は `.tf` （HCL）だけではなく、JSONで記載できる。この場合は、`.tf.json` という拡張子が推奨される。スクリプトなどでコード生成する場合に、JSONの方が扱いやすい場合があるとされる。
