...
| プロトコル     | ✅️ HTTP/SOAP/JDBCなど多数、プラグインで拡張可    | ⚠️HTTP/WebSocket/gRPCなど、拡張可                                  | ⚠️HTTP/WebSocket/gRPC/MQTTなど                                 | ❌️HTTP特化                                                           | ✅️ 多くのプロトコルに対応                                                             |
| テスト準備     | ✅️ GUI記録機能あり、直感的                       | ⚠️ コード記述、har-to-k6等あり                                     | ⚠️ コード記述、レコーダー機能あり                              | ⚠️ シンプルな設定/CLI、複雑なシナリオ不可                             | ✅️ JMeterシナリオ(GUI作成/既存)を再利用                                               |
...
- 数千TPSなど高負荷が求められていない簡易的検証で良ければVegetaを利用する
