...
Idempotency-Keyヘッダという[IETF Draft（2024年12月時点ではDraft 05）](https://datatracker.ietf.org/doc/html/draft-ietf-httpapi-idempotency-key-header-05)がある。このDraftでは、POSTやPATCHに冪等性（同じリクエストが2回来たとしても重複で処理をしない性質）を持たせることが議論されている。主なユースケースは、ペイメントなどで多重の引き落としが許容されない場合である。特にマイクロサービスなシステム構成は要求失敗時のリトライが一般的なため、多重で処理が成功してしまわないように制御する必要がある。
...
カスタムヘッダについて、推奨は以下の通り。

- カスタムヘッダを追加する際は、その用途を明確化する
- カスタムヘッダが増えるとWeb API仕様が複雑になりがちなため、追加は必要最小限とする
- 命名は [スタイル](#スタイル) 章を参照

## 品質値

一部のリクエストヘッダでは、品質値 (Quality Value / q値)に0～1の数値を設定することで、クライアントが取得したい形式やエンコードの優先度を指定できる。
...
「新宿駅　南口」といったキーワード検索をする場合は、 `q=新宿駅%E3%80%80南口` というように（※本来は新宿駅や南口もエンコードされる）、そのままWeb APIサーバ側に連携し、サーバサイドで必要に応じて処理する。フロントエンド側で半角スペースに変換しても良いが、機能配置のポリシーを決め実装が揺れないようにする。
...
Server-Timingヘッダは、サーバー側での各領域の処理時間をクライアントに提供する仕組みである。
...
|      |                    | （1）If-MatchヘッダとETagヘッダ                                                                                    | （2）結果エンティティにおけるETag                                                        | （3）バージョン番号                                                        |
| :--- | :----------------- | :----------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------- | :------------------------------------------------------------------------- |
...
|      | 説明               | 更新前に更新行を再取得し、レスポンスヘッダのEtagでバージョンを取得。If-MatchヘッダにETagで取得したバージョンを設定 | （1）一覧取得のボディからバージョンを取得し、If-Matchヘッダに①で取得したバージョンを設定 | 一覧取得のボディからバージョンを取得し、リクエストボディにバージョンを設定 |
...
2. 更新時はIf-Unmodified-Sinceヘッダに（1）で取得した日時を指定
...
- 社内システムやパートナー向けAPIは、サービスベースを採用する
  - 通常、そこまで多くのマイクロサービスが発生しないと考えるため、API利用者がバックエンドサービスをある程度意識しても問題ないと考える
- 公開APIや、マイクロサービス数が多く認知負荷増大を許容できない場合のみ、リソースベースを利用する
...
- **サーバー保護**
  - 大量リクエストによりサーバの負荷増大で、障害になることを防ぐ
...
- ユーザへのフィードバックなどの観点を含む要件に合わせて、基本的に呼び出し元が必要に応じてキャンセルする思想とする
...
CORS（オリジン間リソース共有、Cross-Origin Resource Sharing）は、あるオリジンで動作しているウェブアプリケーションに、異なるオリジンにある選択されたリソースへのアクセス権を与える仕組みである。追加のHTTPヘッダーを使用して、アクセス権を与えるようブラウザーに指示する。
...
| レート制限（マルチテナントで特定のユーザによるリソース占有の防止） | ユーザー単位（API Key単位）で制御する必要がある。要件上、マネージドサービスで実現できない場合は、[Rate limit](#rate-limitレート制限) 章を参考にする                                                            | ❓️API Gateway                       |
...
Web APIの性能テストでは、指定した同時アクセス数やリクエスト数で負荷をかけ、APIの応答速度・スループット・安定性（ステータスコードが正常か）が期待通りかを確認する。同時アクセス数やリクエスト数はシステムの想定されるピークなどを元に設定する。計測対象は、サーバーリソース（CPU、メモリ、ネットワーク帯域など）なども含めることがあり、サーバスペックの見積もりのインプットにもなる。
...
ただし、画面とWeb APIを一体的に構築していて、クライアント側でバリデーションを設けており、通常はサーバサイドで400エラーが発生しない想定というケースもある。この場合、サーバサイドのバリデーションエラーは何かしらの実装不備である可能性が高いため、状況の把握にWARNログを出す運用も考えられる。
...
サーバ内でリトライを組み込んでいる前提でも発生する、DBや他のWeb APIへの接続失敗や、実装不備（例えば、SQLクエリとスキーマの不一致）などが原因となる。
...
- Web APIサーバについてはCPU、メモリ使用率、ストレージを監視する必要はない（オートスケールさせる想定のため）
- ヘルスチェックなど外形監視を利用する
