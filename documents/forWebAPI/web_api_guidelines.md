...
本ガイドラインは、RESTライクなWeb APIの主な設計手法をまとめ、システム開発プロジェクトにおける設計のベースラインを提供する。
...
破壊的変更を伴うアップデートでは、整合性を保ってAPI利用者に切り替えを強制できる場合は良い。そうでない場合は、複数のバージョンを並行して提供する切り替え期間が必要となる。その際、API利用者に何らかの方法でバージョンを指定してもらう必要がある。

## バージョニング方式

バージョンの表現方法として、以下の3パターンが考えられる。

| 評価観点                     | （1）パス方式                           | （2）HTTPヘッダー方式                                        | （3）サブドメイン方式              |
| :--------------------------- | :-------------------------------------- | :----------------------------------------------------------- | :--------------------------------- |
| 説明                         | URIの一部にバージョン情報を付与する方法 | リクエストヘッダーにバージョン情報を付与する方法             | ドメインで切り替える方法           |
...
| ログ                         | ✅️バージョン番号もログに残りやすい      | ❌️ヘッダーも出力しないとどのバージョンを使ったかわかりにくい | ✅️フルパスを出せば残る             |
...
- ユーザー影響があるような、APIの振る舞いを変更する（例えば、同期でリソースを作成していたのが非同期になる）

なお、API内部の振る舞いの変更には、正確にはバグ修正も含まれる。ただし、通常これは後方互換性の破壊には含めない。バグに依存してクライアントが実装されている場合は、個別調整するかメジャーバージョンを上げる。

## バージョン戦略

後方互換性のある改修のうち、パッチバージョンアップやマイナーバージョンアップと呼ばれるものには以下がある。
...
- 後方互換性の無い改修が行われた場合は、メジャーバージョンアップを行う
...
- 通常、v2、v3などのバージョン追加は管理コストが大きいため、できる限り採用しない
  - 例えば、内部の振る舞いを多少変更する程度であれば、任意のオプションを追加して切り替え可能とする
  - OpenAPI定義上、スキーマが大きく変わるようであればバージョンを上げて分離する

::: tip  
ソースコードはセマンティック バージョニング x.y.z で管理することが多い。各APIのバージョン（v1、v2など）と、ソースコードのバージョン（1.2.1など）は一致せず、全く別のライフサイクルで管理する。  
:::

## マイクロサービスのバージョニング

[マイクロサービス時の振り分け](#マイクロサービス時の振り分け) 節の「（1）サブドメイン方式」を採用する場合、サービスごとのバージョン管理は独立して行うことが自然である。

サブドメイン方式におけるバージョニング例:

- `api.foobar.example.com/v1/articles/42112`
- `api.quux.example.com/v3/orders/8521`

一方、「（2）サブパス方式」を採用した場合、バージョン表現は下表の2方式が考えられる。

| パターン         | （1）サービス単位バージョン                                             | （2）グローバルバージョン                                                                       |
| :--------------- | :---------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------- |
| URL例            | `/orders/v1/list`<br>`/users/v2/profile`                                | `/v1/orders/list`<br>`/v1/users/profile`                                                        |
| 説明             | 各マイクロサービスが自身のバージョンをURLに持つ                         | 「システム全体のバージョン」を定義し、内部の各マイクロサービスのバージョンへルーティングする    |
| 開発の独立性     | ✅️ 高い。<br>各チームは他サービスの影響を受けずにバージョンアップが可能 | ⚠️ 低い。あるサービスの破壊的変更が、システム全体のバージョンアップ（v2）を強いる可能性がある   |
| Gatewayの複雑度  | ✅️ 低い。単純なパスルーティングで済む。                                 | ❌️ 高い。「表向きv1」と「内部のusers-v2」をマッピングする変換定義が肥大化し、運用コストが高まる |
| 利用者への見え方 | ⚠️ サービスごとにバージョンがバラバラに見える                           | ✅️ 統一された単一システムに見えるため、直感的である                                             |

推奨は以下の通り。

- （1）を採用する
  - 直感的な設計になり、利用者側/提供者側で認識齟齬が生じにくいため。また、API Gateway構成 > [ルーティング設計](#ルーティング設計) 節と整合性が揃うため
  - マイクロサービスの最大のメリットである、独立性を妨げないため
  - バージョンマッピングは、サービス数が増えるにつれて設定が複雑化し、運用のボトルネックになりやすいため

## APIの廃止

将来的なWeb APIの廃止予定が決まった場合、OpenAPI定義では、 `deprecated:true` で廃止予定の表明ができる。
...
これにより、Swagger UIのドキュメント上も廃止予定の旨が明示される。

![2025年●月に廃止予定と書かれたWeb APIドキュメント](deplicated.png)

Web APIの応答には、RFC 8594 \- Sunset HTTPヘッダーフィールド（[日本語訳](https://tex2e.github.io/rfc-translater/html/rfc8594.html)）にあるように、非推奨の提示ができる。RFC 8594はInformationalであり標準ではないが、押さえておくべき内容である。

`/v1/orders/{order_id}` が廃止予定になった場合の応答例は下記の通り。
...
- プライベートAPIの廃止は影響度調査を実施し、周知と調整を実施する
- 同時に、OpenAPI定義にdeprecated設定する
- レスポンスヘッダーのDeprecationヘッダーをtrueにする
  - Sunset, Linkは任意で設定する
- クライアントは、Deprecationヘッダーがtrueの場合、WARNログを出すようにするなどの周知を予め行う

実際に廃止予定のAPIを削除する際は、アクセスログなどから利用実績を確認した上で行う。
...
HTTPリクエストメソッドは[RFC 7231](https://datatracker.ietf.org/doc/html/rfc7231#section-4.3)および[RFC 5789](https://datatracker.ietf.org/doc/html/rfc5789#section-2)で定義されている。業務アプリケーションで使用するメソッドは以下の通りとする。CONNECT、OPTIONS、TRACEには言及しない。
...
PATCHメソッドについては [新規APIの実装でPATCHメソッドを使用しないようにしました](https://developers.prtimes.jp/2025/11/11/no-patch-in-new-api/)で報告されているとおり、企業の社内ネットワーク（プロキシサーバー、Firewall、セキュアWebゲートウェイ等） の状態によっては、501 Not Implemented エラーなどでブロックされる事例が確認されている。PATCHだけブロックされるのは、PATCH (RFC 5789) の標準化が 2010年 と他のメソッドより新しいためと推測できる。それ以前に構築された古いミドルウェアが残っている場合、未知のメソッドとして処理される。そのような古い環境を持つ顧客をターゲットにする必要がある場合は、PATCHの代わりにPOSTと `X-HTTP-Method-Override` ヘッダーで代用することを検討する。
...
- URLに表示したくない、ログに書き出したくない値（秘匿情報、個人情報など）を指定したい
  - 暗号化することで緩和可能だが、通常、ログに出ること自体避けるべきである

以下の設計は避ける方針とする。

- GETでリクエストボディ（コンテンツ）にパラメータを指定する
...
1. 新規作成はPOSTを使用する
2. PUTでのリソースの新規作成は基本的に非推奨とするが、理由があれば対応して良い
   - PUTの実装は、SQLの場合はUpdateをMergeにする必要があり手間
   - DynamoDBなどKVSの場合は、登録 / 更新を区別しないほうが楽であるため、新規作成を許容しても良い

1の理由は以下の通り。

- POSTとPUTでは仕様上、冪等性に差があり、同じ作成処理でも挙動差異で混乱する可能性があるため
...
- リソースの更新はPATCHよりPUTの利用を推奨する
  - PUTに統一することで設計 / 開発工数を抑える方針とする
  - PATCHは実装方針が明確に決まっておらず（「PATCHによる部分更新」参考）、設計工数も必要となるため
- 下記の場合はPATCHの使用を検討する
  - クライアントがリソースの全量を持たない
...
HTTPメソッドで表現できない処理を `customVerb` （カスタム メソッド）で表現する場合、以下の3種類の設計案が存在する。
...
- できる限りHTTPメソッドの表現で対応するように努める
  - `batch` `copy` `move` `cancel` `undelete` などをHTTPメソッドでどうしても表現しにくい場合や、パスが既存機能と重複する場合は、カスタムメソッドを導入する
  - 後者は、1件登録の `POST /orders` を作成したあと、後日バッチ登録を追加する場合に `POST /orders/batch` と区別するようなケースを指す
...
- いったんカスタムメソッドを許容すると、適用範囲が広くなってしまう傾向があるため、なるべく利用を抑える
  - 例えば、 `POST /users/1/upgradePlan` が生まれる。これは `PUT /users/1/subscription` など、工夫次第でREST表現できる
...
バックエンドがREST志向ではなく、RPCとして動作させた方が適切なケースもある。例えば以下のような条件がある。
...
- 開発～保守運用まで見据えた場合に、メンバーのスキルセットによって、設計の一貫性を保てない懸念がある場合
  - 例えば、`POST /users/1/updateProfile` といったエンドポイントが作成され、教育 / レビュー体制 / 静的解析で防ぐことが難しいと考えられる場合
...
POST、PUT、PATCH、DELETEを呼び出した場合、そのエンティティの結果をレスポンスボディに含めるべきか否かは設計上の議論になりやすい。一般論として以下の考え方が多い。
...
- フロントエンド側の要件で、リソースを応答した方が都合の良い場合は返す
...
- 公開APIが不特定多数に利用される場合は、安全側に倒す意味でエンティティを返すことも検討する

## DELETEで204 No Content 以外を返すべきかどうか

削除対象の子リソースがカスケード削除された場合に、その件数などを応答すべきではないかという議論がある。

推奨は以下の通り。

- フロントエンド側の要件でリソースを応答した方が都合の良い場合は返す
  - DELETEのレスポンスで「何個消えたよ」という情報が欲しい場合は追加する
...
Idempotency-Keyヘッダーという[IETF Draft（2024年12月時点ではDraft 05）](https://datatracker.ietf.org/doc/html/draft-ietf-httpapi-idempotency-key-header-05)がある。このDraftでは、POSTやPATCHに冪等性（同じリクエストが2回来たとしても重複で処理をしない性質）を持たせることが議論されている。主なユースケースは、ペイメントなどで多重の引き落としが許容されない場合である。特にマイクロサービスなシステム構成は要求失敗時のリトライが一般的なため、多重で処理が成功してしまわないように制御する必要がある。
...
- サーバーサイドのDBには `Idempotency-Key` の値とペイロードのハッシュ値を格納するカラムを追加し、`Idempotency-Key` 側のカラムにユニーク制約を貼る
- サーバーサイドは、`Idempotency-Key`が指定された場合、その値とペイロードのハッシュ値を突き合わせて、どちらの値も一致していれば処理済みとする
- `Idempotency-Key` が処理済みだが、ペイロードハッシュ値が異なる場合は、400 (Bad Request) エラーを返す（フロントエンド側で別のUUIDを発番してもらう）

::: tip 業務アプリケーションで伝統的に行われていた設計パターン
ボタン連打などの対策として、業務アプリケーションでは以下の設計がよく行われていた。
...
- JSON Merge Patchでハマりやすい部分更新が出てきた場合は、PUTで処理できるエンドポイントを設けるなど、PATCHにこだわりすぎない
...
リクエストヘッダーの推奨を下表にまとめる。

| 項目            | 推奨                                                                                                        |
| :-------------- | :---------------------------------------------------------------------------------------------------------- |
| Authorization   | 認証トークンなどは Authorizationヘッダー（またはCookie）に設定する。クエリパラメータやボディには設定しない  |
| Content-Type    | APIが対応しているメディアタイプを指定する（多くは `application/json` になる想定）                           |
| User-Agent      | クライアントが対向システムである場合、利用実績の把握のため `User-Agent: SystemABC/1.0` などと指定しても良い |
| Accept-Language | 多言語対応しているWeb APIの場合は指定可能にする                                                             |

カスタムヘッダーについて、推奨は以下の通り。

- カスタムヘッダーを追加する際は、その用途を明確化する
- カスタムヘッダーが増えるとWeb API仕様が複雑になりがちなため、追加は必要最小限とする
- 命名は [スタイル](#スタイル) 章を参照

## 品質値

一部のリクエストヘッダーでは、品質値 (Quality Value / q値)に0～1の数値を設定することで、クライアントが取得したい形式やエンコードの優先度を指定できる。
...
- `Accept-Language`
  ...
  - 理由
    - エラーメッセージや、リソース（例: 商品名）で特定の言語を優先し、存在しない場合はフォールバック言語（例: 英語を優先、なければ日本語）で返すという用途には有効な場合がある
    - しかし、多言語対応は、JSONの応答項目を英語用・ローカル言語用で分けるなど、アプリケーション層で対応することを第一に検討する
- `TE`: 項目そのものを原則、使用しない
- `Want-Digest`: 項目そのものを原則、使用しない

::: info 参考
[現場で役立つAPIデザイン p23 - Speaker Deck](https://speakerdeck.com/nagix/xian-chang-deyi-li-tuapidezain?slide=23)
:::

# クエリパラメータ

クエリパラメータは以下のURLの `key1=value1&key2=value2` にあたる要素を指す。
...
- Web APIのURLはブラウザのアドレスバーに表示されないことが一般的である。しかし、第三者が一般ユーザーに開発者向けコンソールを開かせるといった攻撃も考えられる。そのため、クエリパラメータにはアクセストークンのような機密情報は載せず、HttpOnly属性を付けたCookieを利用する

## GET/HEAD以外でのクエリパラメータ

RFC 7231（[日本語訳](https://triple-underscore.github.io/RFC7231-ja.html)）を読む限り、POST/PUT/PATCH/DELETEでクエリパラメータを利用すること自体は禁止されていない。
...
- POST/PUT/PATCH/DELETE でクエリパラメータの利用はあまり一般的でない
- 設計が揺れる懸念（下記の例を参照）
...
「新宿駅　南口」といったキーワード検索をする場合は、 `q=新宿駅%E3%80%80南口` というように（※本来は新宿駅や南口もエンコードされる）、そのままWeb APIサーバー側に連携し、サーバーサイドで必要に応じて処理する。フロントエンド側で半角スペースに変換しても良いが、機能配置のポリシーを決め実装が揺れないようにする。

## ソート条件を複数指定

複数のキーに対して昇順 / 降順を指定する方法には多くの設計案がある。代表的な3案それぞれの特徴を下表にまとめる。
...
| curlとの相性                 | ✅️良い                                  | ✅️良い                                        | ⚠️エスケープに--data-urlencodeオプションが必要など多少の手間がある                      |
| 視認性                       | ✅️ソート条件が1箇所にまとまっている     | ⚠️ソート条件が増えると、キーも増える          | ❌️冗長に見える                                                                          |
| パラメータの作成の手間       | ⚠️多少の手間はある                      | ⚠️少し手間                                    | ✅️容易                                                                                  |
| サーバーサイドのパースの手間 | ⚠️多少の手間はある                      | ⚠️少し手間                                    | ✅️JSONであるため楽                                                                      |
...
補足として、上記1から3の設計案には以下のような亜種パターンが考えられる。
...
- 昇降順に±
  ...
  - ただし、「+」は省略し降順のみ「-」をつける方針にすれば、ハマる可能性を減らせる。この場合は特にデメリットが無いため、「-」の有無で昇順 / 降順の指定としても良い
- ソートキーと昇降順の分離
  - sortとorder_byの項目数を一致させる必要があり、OpenAPIのスキーマ定義で記載できない（≒生成コードのバリデーションで、個数の一致チェックができない）

## バッチGETを行う場合

複数IDを指定したバッチ検索の場合は、条件に応じていくつかの設計案がある。
...
Web APIのクエリパラメータ `fields` で必要な項目だけに絞り込むと、無駄なデータ転送を減らせる。一方で、OpenAPIのスキーマを元にしたコード生成との相性を考慮して導入の是非を判断する。例えばGoであれば、ゼロ値の場合はJSON項目に含めない（omitempty）設定が生成コードで可能かを確認する。効果はgzip圧縮した後のペイロードサイズで比較する必要があり、思ったより得られないケースも多い。

推奨は以下の通り。

- 開発工数がかかるため、必要が無ければ導入しない
- 導入する場合も、必要最低限の機能に留める
...
[RFC 7231](https://tools.ietf.org/html/rfc7231#section-6)で定義されているレスポンスステータスコードを利用する。使い分けは、[RFC9205](https://datatracker.ietf.org/doc/html/rfc9205)（[日本語訳](https://tex2e.github.io/rfc-translater/html/rfc9205.html#section-4.6)）の方針に従う。各種エラーに対して適用可能なステータスコードを使用し、疑わしい場合は一般的なステータスコード（200・400・500）を利用する。
...
| 413 Payload Too Large     | リクエストヘッダー、リクエストボディが条件を超えている場合                                                                                                                                              |          | ✅️   | ✅️  | ✅️    |        |
| 414 URI Too Long          | リクエストされたURL長が規定より長い場合。OpenAPIスキーマを用いたチェックに通常まとめられるため、利用しない                                                                                              | ⚠️       |      |     |       |        |
| 422 Unprocessable Content | 入力値が業務処理を行う条件を満たさない                                                                                                                                                                  | ✅️       | ✅️   | ✅️  | ✅️    | ✅️     |
...
| 503 Service Unavailable   | メンテナンスウィンドウである場合、Web APIサーバーの前段のLoad Balancerなどで503を返すことはありえる。DBへの接続失敗など、一時的にアクセスが集中したことに起因する場合は、500ではなく503を使う場合もある | ✅️       | ✅️   | ✅️  | ✅️    | ✅️     |

## なぜ細かくHTTPステータスコードを使い分けるのか

HTTPステータスコードをできる限り細かく使い分けると、以下のメリットがある。

- Web APIの性能テストなど、その機能の開発者以外の担当者が横断的に利用する場合のヒントとなる
  - 400ステータスなので入力不備としてテストケースを直そう、401/403で認証に問題があり負荷を掛けられていなかった、といった情報が分かる
  - 性能テストでは数千TPSなど大量のリクエストを飛ばすため、個別のレスポンスボディの内容を確認するのは難しく、ステータスコードの発生割合を見て実施することも多い
  - ベンチマーキングツールの中には、ステータスコードの発生割合から実施レポートを自動生成するものもある
- ペネトレーションテスト（脆弱性診断）の実施でも同様である
- Web APIを利用したフロントエンド開発者にとっても、セマンティックに従った適切なレスポンスコードは、原因調査の最初の切り分けとして有用である。開発生産性に寄与するためなるべく正しく使い分けることを推奨する

## ユースケース別ステータスコード利用方針

Web APIの設計ではステータスコードの適用に悩むことも多い。本節では、悩みやすいユースケース別に方針を記載する。

| 設計上よくあるユースケース                                                                                 | 方針                                                           | 説明                                                                                                                                                                                                                                                                                         |
| :--------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 存在しないHTTPメソッドを指定された場合、GET/POSTにフォールバックすれば405は利用しなくても済むのではないか | フォールバックせず405を返す                                    | セキュリティ観点から、HTTPメソッドのフォールバックは危険であるため（CORSなどで利用すべきHTTPメソッドを絞る方針）                                                                                                                                                                             |
| 一覧検索で結果が0件の場合は200 、204、404のどれを利用すべきか                                              | 200                                                            | 指定した条件での結果が0件であることは正常な状態であるため200とする。また、一覧検索の多くは結果件数やページング有無を返すため、204は利用できない                                                                                                                                              |
...
| PUTで新規作成した場合と更新した場合で201 Created, 200 OKを使い分けるべきか                                 | Yesだが、200に統一しても良い                                   | PUTで新規作成を許可する方針であれば、201と200を区別したほうが、より標準的である。一方で開発上使い分けが面倒かつ、実務的なメリットも大きくないため、チーム方針で200に統一しても良い。201、200を使い分けることを推奨する                                                                       |
...
| 権限が無いリソースをGETした場合、403 Forbiddenか404のどちらを返すべきか                                    | 404                                                            | 403を返すとデータが存在するというヒントを攻撃者に与えてしまうため、404を返す。社内向けの業務アプリでは、開発者への切り分け情報として403にしたいという要求もまま聞くが、許容しない。社内向けであっても、存在すること自体を隠す必要がある業務がゼロで無い以上は、404で統一する方がベターである |
| 403 の使い所の例を教えて                                                                                   | 参照権限はあるが、更新権限が無いリソースを更新しようとした場合 | 更新権限がないリソースに対して、PUT/PATCH/DELETEを行った場合は、403を利用する                                                                                                                                                                                                                |
| エンドポイントを呼び出す権限自体がなかった場合も403か                                                      | Yes                                                            | ユーザーのロールにエンドポイントを呼び出す権限がない場合は、403を返す。権限が無いWeb APIのエンドポイントが存在すること自体は、APIドキュメントで公開されているはずであるため、呼び出し元が知っても良い                                                                                        |
| その他、微妙なニュアンスで何を指定してよいかわからない                                                     | 200、400、500を用いる                                          | RFC 9205では疑わしい場合は一般的なステータスコード（200、400、500）を用いるとある                                                                                                                                                                                                            |
...
レスポンスボディはJSON形式で応答する。その形式は、以下の指針を持って設計すること。

- **フロントエンドが扱いやすい形に調整する**（例えば、フロントエンド側で縦横変換や粒度調整が必要な場合は、バックエンド側で対応してしまうことが望ましい）。ソート順などもそれに含まれる（場合によってはソート順を指定できるようにする）
  - 通常、バックエンド側で対応するほうが品質保証しやすい
  - Web APIが1つのクライアントからの利用が主体であれば特にそのように意識すること
  - 例えば、RDBの構造でそのまま応答してしまう設計にしがちである。バックエンドとして特定のフロントエンドの要件に引っ張られてREST志向（リソース志向）を崩しすぎない範囲で、利用者視点で使いやすい形式を模索することは必要である
- 一覧検索などで配列を応答する場合、配列をラップし、JSONオブジェクトの属性とする。検索件数やページング情報も含ませる（※後述）

## 一覧検索などで配列を応答する場合の形式

DBなどから取得したレコードを配列のまま返すのではなく、一律オブジェクトでラップして返す。
...
|            | クライアント側でitems項目を取得するというひと手間が必要だが、大した労力ではない |
...
- 実装上の不具合が無く、OpenAPI定義としても `null` 許容とすると手間がかかるため

✅️推奨: scoreの値が存在しない場合、キー自体を含めない
...
日付項目はUNIXタイムスタンプを返すか、ISO 8601で返すかの考慮がある。
...
- モバイル対応予定が無く、プライベートなAPIであればコード値のみを返す
  - 区分値がDBにコード値で格納されている場合、値とのマッピングなしでDBからの取得結果をレスポンスにでき、開発をシンプルにできる
...
エラー情報の詳細（例えば、バックエンドの言語やフレームワークのスタックトレース）を、呼び出し元にフィードバックするかどうかは、公開API / 内部APIによって大きく判断が分かれる。

| 条件                      | 推奨                                                                                                                                                                                                                                       | 補足                                                                                   |
| :------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------- |
| 公開APIの場合 （特にB2C） | 攻撃者にヒントを与えることになるため、利用している言語 / フレームワーク / ライブラリ / 各種バージョンなどの情報を出さない。クライアントに返すエラーメッセージは簡潔かつ必要最低限の情報にとどめる                                             | 「DBアクセスに失敗しました」など簡潔なメッセージとする                                 |
...
Web APIのサーバーサイドでバリデーションエラーが発生した場合に、どの項目に不備があったかユーザーにフィードバックするため、JSON PATHでエラー箇所を返すと良い場面がある。
...
- エンコーディングの指定は不要（RFC 8259から、JSONのエンコードはUTF-8のみであるため）
...
Server-Timingヘッダーは、サーバー側での各領域の処理時間をクライアントに提供する仕組みである。

cache取り出しに23.4ミリ秒、dbクエリに50ミリ秒、appロジックに75.3ミリ秒かかったことを示す例を挙げる。
...
- チューニングが必要なAPIに絞って、Server-Timingヘッダーを使用する（予め全APIに組み込むのは費用対効果が悪い）
...
[Cache-Control](https://developer.mozilla.org/ja/docs/Web/HTTP/Headers/Cache-Control) を用いると、ブラウザやCDNやプロキシなどに対してキャッシュ制御の指示を出せる。

推奨は以下の通り。

- 基本方針としてはレスポンスヘッダーに `Cache-Control: no-store` を追加し、キャッシュを禁止する
  - 業務システムでは通常、権限設定がなされている。キャッシュにより機密情報が不用意に参照できてしまうことを防ぎたいため
...
- 区分値取得APIなど、キャッシュ可能なものは個別判断でキャッシュを有効にする
  - 後続のコンピューティングリソースを稼働させないことでクラウド費用を下げられるため、共有可能な情報であればキャッシュを有効にすることが望ましい
...
セキュリティ関連で使用すべきレスポンスヘッダーが存在する。[セキュリティ](#セキュリティ) の章を参照。
...
- クライアント側で入力チェックを行っていたとしても、サーバーサイドでの入力チェックも同様に行う

理由は以下の通り。

- クライアント側の入力チェックは、UXのためのものである。即時にユーザーへフィードバックし、何が悪かったかを示すためにある
- サーバーサイドのチェックは、システムを保護するために存在し、例えば攻撃者がブラウザを介さず想定外の入力値を連携することを防ぐ

# 区分値取得APIの是非

フロントエンドでプルダウン項目などを生成するため、区分値（参照データとも呼ばれる）を取得するAPIを作成する場合がある。これについては以下の設計案が考えられる。

| 観点            | （1）区分値APIを用意する                                                    | （2）フロントエンド側でハードコードして持つ                                            |
| :-------------- | :-------------------------------------------------------------------------- | :------------------------------------------------------------------------------------- |
| 説明            | 画面初期描画時に、必要な区分値をAPIを経由して取得する                       | フロントエンドのビルド時にリソースファイルから、ハードコードして保持する               |
| デプロイ 難易度 | ✅️DBに区分値の追加。またはサーバーデプロイ                                  | ⚠️1のデプロイに加え、フロントエンドのデプロイが必要。                                  |
| モバイル 対応   | ✅️バックエンドのリリースで区分値追加 / 削除が可能                            | ❌️アプリ審査のためリードタイムが必要（※MDM配布の場合は緩和され✅️になる）               |
| サーバー負荷    | ⚠️わずかとは言え、通信が発生                                                | ✅️サーバー負荷が無くなる                                                               |
| ユーザー体験    | ⚠️わずかとは言え、初期表示が遅延。NW状況によっては不利                      | ✅️区分値のデータサイズはたかが知れており、一度ロードしてしまえば初期表示が高速化される |
...
「DB設計標準」より、排他制御のためにDB側の各テーブルにはバージョン番号が存在することを前提とする。この場合にREST APIで楽観ロックを実現する方法は、下表の通り。
...
::: warning 楽観ロックかつ、1件更新のAPIを提供する場合は、If-Matchヘッダーで実装すべきか？
...
::: tip Last-Modifiedヘッダー を利用した楽観ロックは利用しない
...
| Cons | ❌️リクエストヘッダーを改ざんして、容易に上書き可能<br>❌️バルク更新に対応不可<br>❌️APIサーバー側が複数インスタンスから構成される場合、厳密な時刻同期がなされていないと、正確に楽観ロックできない懸念<br>（※最終更新日をDBではなくWeb API側で取得する前提） |
...
sequenceDiagram
    ...
    participant Server as Web APIサーバ
    participant Storage as オブジェクトストレージ

    Client->>Server: 署名付きURL発行依頼
    Server->>Server: 署名付きURLの発行
    Server-->>Client: 署名付きURL

    Client->>Storage: ファイルアップロード
    Storage-->>Client:

    Client->>Server: ファイルメタデータ登録
    Server-->>Client:
```
...
- サムネイルなど小さなデータの場合（数KB程度を想定）は、Base64でエンコードし、JSON項目の値に設定して返す
  ...
  - 例えば、`<img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUA..." alt="サムネイル画像">` のようにsrcへ設定できる形式である
...
- データ量が多く一度に全データを返却するとサーバーやクライアントの負荷が高くなる場合
  - 数百万件の商品リスト
  - 購入履歴など
- 新規データが次々と追加される状況でリアルタイム性を損なわずにデータを取得したい場合
  - SNSのタイムライン
  - トランザクションログ
- ユーザーが全てのデータを一度に必要とするケースが稀で、多くの場合最初の数件だけが表示されれば十分な場合
  - ECにおける商品の検索結果
  - メッセージアプリにおけるチャット履歴
...
特にSSKDsなAPIでは、API提供者がAPIクライアントを管理できることが多く、設計上の工夫でページングを不要にできるケースが多い。

例えば、データ量が多い状況でも、次のような場合はページングを不要とできる。

- 検索条件による絞り込みを強制できる場合（例. 最長1ヶ月のレンジでFROM/TOの指定が必須）
- 上限1000件など返却可能な件数を決め打ちしておき、万が一上限以上のデータが存在する場合は検索条件による絞り込みをUIとして促せる場合
...
Rate Limit（レート制限）とは、特定の時間内に許可されるリクエストの数を制限する仕組みである。以下のような目的がある。

- **サーバー保護**
  - 大量リクエストによりサーバーの負荷増大で、障害になることを防ぐ
  - 悪意が無くても、連携先の実装不備により大量リクエストが届きうる
- **公平なリソース配分**: 一部ユーザーがリソースを占有することを防ぐ
- **セキュリティ**: DDoS攻撃や、不正なアクセスを試みるボットなどからサービスを保護する
- **コスト管理**: 使用量に応じて課金が発生する場合に、上限を設けることで予期しないコスト増加を防ぐ

推奨は以下の通り。

- Rate limitを設ける
- Rate limitはクラウドサービス（WAFやAPI Gateway）側で行う
- マルチテナントのユーザー単位（API Key単位）の制御をスクラッチで実装する場合は、以下の点を注意する
  - 正確なカウントはDB負荷が高いため、Cacheサーバーの利用（定期的にWrite backでメインのDBに書き込み）方式を検討する
  - Rate limitの場合は `429 Too Many Requests` を返し、`Retry-After` ヘッダーを返すこと
...
タイムアウトを適切に設定することで、リソースの無駄遣いを防いだり、クライアントの待機時間によるストレスを軽減できる。
...
- ユーザーへのフィードバックなどの観点を含む要件に合わせて、基本的に呼び出し元が必要に応じてキャンセルする思想とする
- 非同期APIで作成された非同期タスクについては、キャンセルするユーザーがいないため、あらかじめ決めた最長実行時間に応じてタイムアウトさせる
...
「フロントエンド認証フロー」によって、アクセストークンを取得できたとする。Web APIへの認証には大別して以下の2つの方式がある。

<div class="img-bg-transparent">

|                     | （1）ベアラートークン方式                                                                                                                                                                     | （2）セッショントークン方式                                                                                                                                                |
| :------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| フロー図            | [![uml][bearer_token_flow_img]][bearer_token_flow_link]                                                                                                                                       | [![uml][session_token_flow_img]][session_token_flow_url]                                                                                                                   |
| 説明                | APIリクエスト毎にアクセストークンをAuthorizationヘッダーに含める方式。トークンの検証はIdP側のトークンイントロスペクションを利用                                                               | 初回リクエストのみアクセストークンを利用し、以降はセッション管理（セッションIDをCookieに含めて送信）による認証を行う方式。セッション管理テーブルやキャッシュサーバーが必要 |
| 処理性能            | ❌️リクエストの度に、トークンイントロスペクションAPIで確認が必要。アクセストークンのデータサイズによっては多少不利                                                                             | ✅️IdPへの問い合わせが不要。ただしセッション管理にはKVSなど高速なストアが求められる                                                                                         |
...
::: tip 廃止になったオプション  
[X-XSS-Protection](https://developer.mozilla.org/ja/docs/Web/HTTP/Headers/X-XSS-Protection) や [X-Frame-Options](https://developer.mozilla.org/ja/docs/Web/HTTP/Headers/X-Frame-Options) などContent-Security-Policyの登場で非推奨となったヘッダーも多い。  
:::

## CORS

CORS（オリジン間リソース共有、Cross-Origin Resource Sharing）は、あるオリジンで動作しているウェブアプリケーションに、異なるオリジンにある選択されたリソースへのアクセス権を与える仕組みである。追加のHTTPヘッダーを使用して、アクセス権を与えるようブラウザに指示する。

同一システム内であっても `app.example.com` `api.example.com` でサブドメインが分かれることもあり、別システム向けにWeb APIを公開しない場合も考慮が必要になりうる。

同一システム内の対応としては、以下の考え方がある。

| 観点         | 1.同一オリジン構成                                                                                                                                                        | 2.複数オリジン構成                                                                                                                          |
| :----------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------ |
| 説明         | 静的リソースとWeb APIを同一オリジンでホストし、CORSがそもそも発生しない構成にする。AWSであればLBやCloudFrontのパスベースルーティングで、S3とWeb APIを振り分ける構成にする | `app.example.com` `api.example.com` の2つのドメインでそれぞれ管理する。システムの独立性を高められるが、管理するドメインが増える可能性もある |
| CORS考慮     | ✅️不要                                                                                                                                                                    | ⚠️考慮が必要                                                                                                                                |
...
Web APIは基本的にステートレスであるべきである。そのため、クライアントの状態をサーバー側で保持するためのCookieは避けるべきである。

[認証](#認証) 章の 「2.セッショントークン方式」 を利用する場合のみ、ログイン成功後にセッションIDを含むCookieを発行する。

`Set-Cookie` ヘッダーにはいくつかの属性があり、セキュリティ観点で適切に設定する必要がある。
...
- **\_\_Host-**
  ...
  - サブパスの場合は、上書きすること自体は可能だが `/` 固定になるため、特定のパスだけ異なったCookieの値が使用されることはなくなる。セッション固定などの攻撃により耐性がある

仮に Web API のパスが `/api/` であった場合には2通りの設定が考えられる。

1. **Pathの絞り込みを優先パターン**
   - 例: `Set-Cookie: __Secure-ID=123; Secure; HttpOnly; Path=/api/`
   - Pathを `/api/`　に絞ることを優先するため、 `__Secure-` プレフィックスを利用
   - ✅️セッション送信先を限定できる
   - ❌️Domain属性を指定してしまうという設定ミスは防げない、その場合はサブドメインからCookieが不正に上書きされる恐れ
...
- セッショントークン方式の認証以外の用途では、原則、Cookieを使用しない
...
- `Set-Cookie` には以下の属性を設定する
  ...
  - `Path` 属性: `/` を指定する。`__Host-` プレフィックスを利用したいため
  - `Domain` 属性: 原則、省略（発行元のホスト名に限定）する。サブドメイン間で共有は行わせないため
  - `Max-Age` 属性: 合意されたセッション有効期間を指定する
  - `Expires` 属性: `Max-Age` を用いるため、利用しない
  - `__Host-` プレフィックスを利用する。送信スコープより、設定値の保護を優先する方がリスクは低いと考えるため
- ログイン時は常に新しいセッションIDを再生成する（既存の値を流用しない）
- ログアウト時に、サーバーのセッション情報の破棄とともに、クライアント側のCookieも `Max-Age=0` を指定して削除する

::: info 参考

- [HTTP Cookie の使用 - HTTP | MDN](https://developer.mozilla.org/ja/docs/Web/HTTP/Guides/Cookies)
...
Web APIには前章までで説明したように実用のために必要な要素が多数あるが、AWSなどのクラウドのマネージドサービスで代用できるものも多い。できる限りマネージドサービス側に寄せることで、設計開発をショートカットでき、差別化領域に集中できる。
...
| レート制限（マルチテナントで特定のユーザーによるリソース占有の防止） | ユーザー単位（API Key単位）で制御する必要がある。要件上、マネージドサービスで実現できない場合は、[Rate limit](#rate-limitレート制限) 章を参考にする                                                              | ❓️API Gateway                       |
| Slow DoSなど                                                         | ペイロードを断続的に送る攻撃への対策。例えば、長大なHTTPリクエストボディ（POST ペイロード）を断続的に送ってセッションを占有する                                                                                  | ❓️Amazon CloudFront ✅️WAF           |
| リクエストサイズの上限                                               | リクエストヘッダー、リクエストボディに巨大なデータを送るDDoS対応。「Large Payload Post」「HTTP爆弾」とも呼ばれる。Amazon API Gatewayはペイロードサイズの上限が制約として10MBで多少緩和できるが、通常は大きすぎる | ✅️WAF                               |
| レスポンスのgzip圧縮                                                 | レスポンスのJSONをgzip圧縮することでユーザー体験を向上させる。特にモバイルの場合はユーザーの通信量を抑えるメリットが大きい                                                                                       | ✅️API Gateway ✅️CloudFront どちらか |
| CORS                                                                 | [CORS](#cors) 章を参照                                                                                                                                                                                           | ✅️API Gateway                       |
| SQLインジェクション/XSS 緩和策                                       | 不正な文字列の検知。ただし、アプリケーション側もSQLインジェクションについては、プレースホルダーの利用、XSSについてはHTMLタグのエスケープを実装する必要                                                           | ✅️WAF                               |
...
Web APIの性能テストでは、指定した同時アクセス数やリクエスト数で負荷をかけ、APIの応答速度・スループット・安定性（ステータスコードが正常か）が期待通りかを確認する。同時アクセス数やリクエスト数はシステムの想定されるピークなどを元に設定する。計測対象は、サーバーリソース（CPU、メモリ、ネットワーク帯域など）なども含めることがあり、サーバースペックの見積もりのインプットにもなる。

負荷ツールの選定基準と比較は、[はじめての性能テスト](/documents/forPerformanceTest/performance_test.html#テストツールを選定する) を参照。

# リリース

## デプロイメント環境

Web APIの環境分離では、クラウド環境のベストプラクティスとして1環境1アカウントにする方法がある。この前提であれば、デプロイメントごとの分離は、ドメイン名で識別可能とする方針が望ましい。例えば、`example.com` と `dev.example.com` などでの分離である。
...
ただし、画面とWeb APIを一体的に構築していて、クライアント側でバリデーションを設けており、通常はサーバーサイドで400エラーが発生しない想定というケースもある。この場合、サーバーサイドのバリデーションエラーは何かしらの実装不備である可能性が高いため、状況の把握にWARNログを出す運用も考えられる。

推奨は以下の通り。

- 400系のエラーはINFOログ出力で良い
- プライベートAPIかつ、画面と一体的に開発している場合には、バリデーションエラーはWARNログを出して、状況把握に努める

#### 500番台

サーバー内でリトライを組み込んでいる前提でも発生する、DBや他のWeb APIへの接続失敗や、実装不備（例えば、SQLクエリとスキーマの不一致）などが原因となる。

推奨は以下の通り。

- 実装不備が起因の場合、エラーログを出力する
- プライベートAPIで、サービスが小さい場合において、DBや外部サービスへの通信エラーが起因となる500を返す場合、エラーログを出力する
  - ユーザーに不具合を返したのであれば、開発者側でも状況を把握し、必要なアクションをすべきである
...
サービスが成長し、例えば常時数百～数千\[TPS\]のリクエストがあるWeb APIでは、1件1件のエラーを調査すると、必要な開発が滞ってしまい全体最適な投資が行いにくくなるという懸念もある。このような場合は、SLI/SLOという考えを導入し、どのくらいの割合まで500エラーを許容するかといったエラーバジェットを検討する。

### メトリクス監視

推奨は以下の通り。

- Web APIサーバーについてはCPU、メモリ使用率、ストレージを監視する必要はない（オートスケールさせる想定のため）
- ヘルスチェックなど外形監視を利用する

## トレースIDをどこで払い出すべきか

クライアント / サーバーサイドのどちらでトレースIDを発行すべきか悩む場合がある。

| 観点               | （1）クライアント側生成                                               | （2）サーバーサイド側生成                                                                        |
| :----------------- | :-------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------- |
...
- 業務アプリケーション開発など、クライアント側のアプリケーションの統制を行えるのであれば、（1）クライアント側でトレースIDを生成する

::: tip エラー問い合わせの自動化  
サーバー問い合わせの結果がNGだった場合のエラー表示で、トレースIDがあれば便利である。もし、ユーザー自身に問い合わせをさせるのであれば、トレースIDの桁数は7桁など郵便番号程度に抑えることが望ましい。トレースIDは重複しても調査時に大きな障害とならないためである。
...
なお、ヘルスチェックのRFCは、Informational（標準化トラックではないが有用な情報）を目指してDraftが作られていたものの、2020年4月に失効している。
