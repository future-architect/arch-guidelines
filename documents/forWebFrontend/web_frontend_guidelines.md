...
- データの性質（一時的か永続的か、共有範囲、セキュリティ要件）に応じて、オンメモリとWebストレージ（localStorage/sessionStorage）を使い分ける
...
アプリケーション全体のごく一部のみにオフライン機能を提供する場合は、スタンドアローン化を必ずしも行わない方針が考えられる。しかし、オフライン対応ページへの遷移手段がブックマークやURL直接入力などに限られてしまう。したがってシステム外でのユーザー手順を訓練しづらいto向けサービスやSaaSではスタンドアロン化を推奨する。
...
- 表示項目を減らすなどの方針で対応できない場合は、（1）の方式の導入を検討する（（1）～（3）の中で工数が低くなると考えられるため）
...
- カラーモードをサーバーサイドでも利用したい場合（サーバーサイドでカラーモード特有のコンテンツを応答したい場面など）は、（2）の利用を検討する（（1）でもLocal Storageから読み出した設定値をサーバーサイドに送信することは可能だが、Cookie送信の方が自動的に送信されるため考慮事項を減らせると考えられるため）
...
- fixable なものはアーキチーム側で一括で適用する
- fixable でなくても、コード品質が向上しレビュー負荷が軽減するのであれば導入するが、そうでないものは必ずしも追加しない
- 既存の検知項目が多く対応が難しい場合は、いったん disabled で無視し、新規分のみに適用する
...
- アドホックにLinterを追加すると、作業計画・要員計画が難しくなるため、定期的にLinter設定を見直すような予定を、PJ計画に組み込んでおく
  - 例えば、四半期に一回ペースで見直しや、先行開発の前後・PG開始前・JT開始前など
