...
昨今のWebフロントエンド領域は、単なるHTML、CSS、JavaScriptでのページ制作から、React、Vue.jsなどのモダンなフレームワークを活用した大規模かつ動的なWebアプリケーションの構築へと変化している。これにより、開発の効率化とユーザー体験（UX）の向上が図られる一方で、設計の考慮ポイントも多様化している。また、アクセシビリティなどより多面的な品質も求められている。
...
また、本ガイドラインは基本的に、業務アプリケーションを想定して作成している。例えば、大規模toCサービスのようにキャッシュを最大限に活用できないという制約がある。
...
なお最もシンプルな方式は、S3単体で静的Webサイトをホスティングすること（ウェブサイトエンドポイント）である。この方式は簡易的な検証などでは利用可能であるが、以下の機能上の制約があるため、本ガイドラインにおける実用的なホスティング方式の比較対象からは除外する。
...
| Pros         | ✅️開発速度の向上<br> ✅️組み込みのアクセシビリティ対応<br> ✅️クロスブラウザ互換性 <br>✅️豊富な開発ドキュメント<br> ✅️大規模なコミュニティサポート<br> ✅️確立されたデザインパターンへの準拠&一貫性の確保                                                                                        | ✅️純粋なスクラッチと比較して開発を加速しつつ、高度な制御を維持する為、デザイン自由度とロジックやアクセシビリティ活用をバランスよく享受可能<br> ✅️既存資産活用や中程度のカスタマイズで実現可能                                                                                                                                                                                                                                                                                                                                                                                | ✅️HTML/CSS/JSに対する完全な制御<br> ✅️最適化されたパフォーマンス（未使用のコード/機能の極小化）<br> ✅️自社ブランドを踏襲したデザイン方針への完全な適合 <br>✅️コンポーネント内部に対するチームの深い理解<br> ✅️ベンダーロックインの回避               |
...
- **UXの向上**: 最新の機能やパフォーマンス改善が自動的に提供されるため、常に快適なブラウジング体験が得られる
...
キャッシュ（Cache）は、データを一時的に保存・再利用することで、データ取得・通信に伴う処理の負荷を軽減し、性能を向上させる仕組みである。キャッシュを適切に活用することでUXの改善、サーバー負荷の低減が期待できる。
...
- セキュリティ
  - 機密データがキャッシュされることで漏洩リスクが生じるため、そもそも保存しない（`Cache-Control: no-store`を使用）ようにしたり、適切な暗号化などを行う必要がある
  - ログアウト時にキャッシュを破棄する等の対応も有効である
- データの一貫性と整合性
  - 意図せず古いキャッシュが使用されることを防ぐために、キャッシュの保持期間を適切に設定する必要がある
  - ブラウザキャッシュ: バージョニングやキャッシュバスティングを活用することでキャッシュの利用有無や有効期限を管理できる
  - アプリケーションキャッシュ: データの同期処理や更新フローを適切に設計することが重要となる
