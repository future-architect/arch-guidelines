...
フロントエンドにおけるコンポーネントは共通コンポーネントと業務コンポーネントを明確に区別が可能である。これらの区別は、効果的なコード編成、再利用戦略、およびライフサイクル管理にとって不可欠である。
...
- 前向きな表現
  - 「間違っています」ではなく、「もう一度入力してください」のように、否定したりユーザーを責めるのではなく、再入力を促す表現にする
...
| 手順 | フロントエンド作業                                           | サーバサイド作業                                                                  |
| :--- | :----------------------------------------------------------- | :-------------------------------------------------------------------------------- |
| 1    | フロントエンド側で必要な情報はすべてサーバー側に問い合わせる | Cookieを持っていない場合はリダイレクトをしてIdPのログインフローをキックする       |
| 2    |                                                              | IdPのコールバックを受けてCookieを発行                                             |
| 3    |                                                              | ログアウトではCookie削除                                                          |
...
静的解析を活用すれば、コードのチェックや適用を自動化できるため、属人性を排除し、安定した開発プロセスを実現できる。例えば、import の順序やパスの書き方といったコード構成のルールも、静的解析を通じて明文化が可能となる。
