...
Web APIの設計については、[Web API設計ガイドライン](https://future-architect.github.io/arch-guidelines/documents/forWebAPI/web_api_guidelines.html) を参考にすること。
...
なお最もシンプルな方式は、S3単体で静的Webサイトをホスティングすること（ウェブサイトエンドポイント）である。この方式は簡易的な検証などではありえるが、実用には次の点で耐えられないと考えられるため、比較からは除外する。
...
- インターネットアクセスが許容される不特定多数向けのSPAの場合、（1）を選択する
  - （２）は制約が多いためSPAホスティングとしては不向き
- プライベートアクセスを前提とするSPAの場合、（3）を選択する
...
| :--------------------------- | :----------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------- |
| 説明                         | コンテナ等で常駐型のNodeサーバーを構築する方式                                                               | S3をオリジンとし、インタフェース型VPCエンドポイントを                    |
...
| 実現方式              | React、Vue.js、Angular                                                                                                                                                                                                  | Next.js、Nuxt.js、Angular Universal                                                                                  |
...
現代のWebアプリケーションは、複雑なUIと多様な機能を備えることが求められており、従来のモノリシックなフロントエンドでは、複数機能のUIやデータアクセスレイヤーが1つのアプリケーションに統合され、複数の開発チームによって変更される単一のコードベースとして存在し、各機能が密に結合している為、管理と拡張が困難になっている。

コンポーネントベースアーキテクチャは、UIをより小さく、独立した再利用可能な部品に分割することで、モジュール性、保守性、テスト容易性により、大規模なアプリケーション開発においてもチーム間の統制を保ち、効率的に実装できるソリューションを提供する。
...
- **共通コンポーネント**
  ...
  - ユーザーインタフェースの基盤を形成し、一貫した外観と操作性を提供する
...
共通コンポーネントについては実際のプロジェクト要件や組織構成、保持資産等を総合的に考慮して下記マトリックスを基に適切なコンポーネント実装方針を策定する。
...
※2025年3月に[StatCounter](https://gs.statcounter.com/browser-market-share/desktop-mobile-tablet/worldwide/)を利用した。Market Share Typ:Browser(Desktop,Mobile,Tablet,Console)、Region:Worldwide、Period:From Feb 2024 To Feb 2025
...
- デバイス毎の利用シェア率、OS別サポート状況を考慮してサポート対象を調整できないか検討・調整する
...
- **ユーザー体験の向上**: 最新の機能やパフォーマンス改善が自動的に提供されるため、常に快適なブラウジング体験が得られる
...
以下の点を総合的に検討し、プロジェクトにとって最適なバランスを見つけることが重要である。

- 状態の特性とスコープ
  - 管理対象となる状態の種類（ローカル状態、共有状態、サーバーキャッシュ状態）とその割合、更新頻度、状態間の依存関係の複雑性、状態が影響を及ぼす範囲などを考慮する
- アプリケーションの規模と複雑性
  - 現在のコンポーネント数や機能数、そして将来的にアプリケーションがどの程度拡張・複雑化する見込みがあるか
- チームのスキルセットと経験
  - 開発チームメンバーのフレームワーク標準機能への習熟度、特定ライブラリの使用経験、そして新しい技術に対する学習コストの許容度
- 非同期処理とデータフェッチ要件
  - API連携の頻度と複雑さ、キャッシュ・バックグラウンド更新・同期・楽観的更新などの必要性、ローディング・エラー状態管理の効率化要求など
- セキュリティ
  - 状態として管理する情報に機密情報が含まれる場合はアクセス制御が必要となる

本ガイドラインとして推奨する状態管理のアプローチと、選定・設計時の考慮点は以下の通り。
...
ウェブアプリマニフェストの設定は比較的簡易であるから、オフライン対応していることをユーザーにわかりやすく示すため（オフライン状態でブラウザを開くことが直感的でないと感じるユーザーも多い）にも、できる限り対応を行うのが好ましい。
...
ブレークポイントとは画面サイズに応じて コンテンツの表示レイアウトを変化させるポイントのことである。CSSを用いたブレイクポイントは、下表の2つの方式のどちらかで設定される。
...
❌️過剰なブレークポイント（コンテナの幅（cqw）と高さ（cqh）に基づくブレイクポイントを過剰に設定した例。ここまで必要なケースは稀だと思われる）
...
| 手順 | フロントエンド作業                                                                        | サーバサイド作業                                                                  |
| :--- | :---------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------- |
| 1    | 認証とJWTトークン取得、トークンリフレッシュなど（ライブラリを使うなら起動時の呼出のみ）   | リクエスト時のJWTトークンを検証する。トークンがなかったり不正な場合はエラーを返す |
| 2    | サーバーリクエスト時にJWTトークンを付与して送信                                           | 検証のためにIdPの証明書を取得                                                     |
| 3    | ログアウトはIdPのログアウトAPIを呼び出しつつ、自分の状態のリセット                        | IdP提供の情報を使う場合はトークンEndPointで定期的に最新のトークンを取得し確認する |
| 4    | JWTトークンをパースして必要な情報を得る。必要な情報がなければ追加でサーバーに問い合わせる |                                                                                   |
...
| 手順 | フロントエンド作業                                           | サーバサイド作業                                                                  |
| :--- | :----------------------------------------------------------- | :-------------------------------------------------------------------------------- |
| 1    | フロントエンド側で必要な情報はすべてサーバー側に問い合わせる | クッキーを持っていない場合はリダイレクトをしてIdPのログインフローをキックする     |
| 2    |                                                              | IdPのコールバックを受けてクッキーを発行                                           |
| 3    |                                                              | ログアウトではクッキー削除                                                        |
| 4    |                                                              | IdP提供の情報を使う場合はトークンEndPointで定期的に最新のトークンを取得し確認する |
...
| 手順 | フロントエンド作業                              | サーバサイド作業                                                                                                             |
| :--- | :---------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- |
| 1    | JWTトークンをパースして必要な情報を得る         | リクエスト時のJWTトークンを検証する。トークンを持っていないか、不正な場合はリダイレクトをしてIdPのログインフローをキックする |
| 2    | サーバーリクエスト時にJWTトークンを付与して送信 | IdPのコールバックを受けて、フロントエンド側の必要な情報をクレームに入れたJWTトークンを発行                                   |
| 3    | ログアウトは自身の持つトークンを削除            | JWTトークンの署名や検証用の証明書の管理                                                                                      |
| 4    |                                                 | IdP提供の情報を使う場合はトークンEndPointで定期的に最新のトークンを取得し確認する                                            |
...
| 手順 | フロントエンド作業           | サーバサイド作業             |
| :--- | :--------------------------- | :--------------------------- |
| 1    | 超大変である。詳細は省略する | 超大変である。詳細は省略する |
...
| :--- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
| Pros | ✅️JWTトークンにある情報であればサーバーに問い合わせずに住む                                                                                                                                                                                                                                                                                                            | ✅️Cookieを使うので必要以上に多くの情報を露出させずに済む<br> ✅️サーバー側のセッションストレージを活用しやすい <br>✅️ログアウトが簡単 | ✅️自前でJWTトークンを作れるため、必要な情報を盛り込める <br>✅️ログアウトが簡単 | ✅️自由度が高い                                                                                                         |
...
フロントエンドにおける認可の目的は「ユーザー体験」の向上であり、ユーザーが権限のない操作を誤って行わないよう、行動を制限することが目的となる。セキュリティの観点からは意味をもたない（ユーザーがコードを改ざんできてしまう）ため、セキュリティの観点からは必ずバックエンド側で認可を実現する必要がある。
...
- すでにnginxが導入されており、追加の学習コストなどに抵抗がなければ（2）
...
静的解析はCI（継続的インテグレーション）との連携によっても活用される。例えば、Pull Request 作成時に静的解析を GitHub Actions などで自動実行し、問題が検出された場合はマージをブロックするような運用が可能である。
...
【参考】例）下図の（1）→（5）の一連の流れはコンポーネントテストで確認する。
