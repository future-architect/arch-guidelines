...
ハイブリッドレンダリングとは、複数のレンダリング手法（主にSSRとCSRを組み合わせ、パフォーマンスとユーザエクスペリエンスの最適化を目指すレンダリング手法である。アプリケーションの異なる部分（例えば、特定のページやコンポーネント）や、異なる状況（例えば、初回ロード時とその後のユーザインタラクション時）に応じて、最適なレンダリング戦略を柔軟に選択し適用できる点が大きな特徴といえる。例えば、SEO が特に重要なランディングページや製品詳細ページには SSR を適用し、ユーザの操作頻度が高いダッシュボードのような部分には CSR を適用するなど、きめ細やかな最適化が可能になる。
...
| 方式 | 説明                                                                                                                                                                                                                                                              |
| :--- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SSG  | Webページを事前に生成しておき、ユーザがアクセスしたときにサーバーから生成済みの静的なHTMLファイルを提供する手法を指す。ビルド時にコンテンツがレンダリングされ、それ以降はその静的ファイルをそのまま提供するため、非常に高速なパフォーマンスを実現できる。         |
...
現代のWebアプリケーション開発では、複雑なUIと多様な機能への対応が求められている。従来のモノリシックなフロントエンド開発では、UIやデータアクセスといった複数機能が一つの大きなコードベースに統合されていたため、機能間の結合度が高く、複数の開発チームが単一のコードベースを変更することになるため、保守運用が困難になりやすいという傾向があった。これに対し、コンポーネントベースアーキテクチャはUIを機能ごとに小さく独立し、再利用可能な部品へ分割することで、保守性・テスト容易性を高め、大規模なアプリケーション開発においてもチーム間の統制を保ちやすいとされる。

フロントエンドにおけるコンポーネントは、その役割や特性のうち外から「共通コンポーネント」と「業務コンポーネント」に区別することが多い。
...
業務コンポーネントは、特定の機能またはビジネスドメインに特化して作られるので、それぞれの要件を満たした固有のビジネスロジックを含んでいる。UIとビジネスロジックをできる限り分離することで、再利用性と保守性を高めやすいとされる。
...
前提として、昨今のフロントエンド開発はフレームワークの利用を中心とした実装が主流であり、命名規則やディレクトリ構造は採用するフレームワークに依存する部分が大きい。その為、本ガイドラインにおいても基本的にはフレームワークが推奨する規則・構成を推奨とする。
...
**参考** [2024年版 HTMLで作るフォームバリデーション \- ICS MEDIA](https://ics.media/entry/240418/)
...
**参考** [反感を買うエラーメッセージのパターン](https://u-site.jp/alertbox/hostile-error-messages)
...
キャッシュ（Cache）は、データを一時的に保存・再利用することで、データ取得・通信に伴う処理の負荷を軽減し、性能を向上させる仕組みである。キャッシュを適切に活用することでユーザー体験（UX）の改善、サーバー負荷の低減が期待できる。

ただし、キャッシュ設定や運用を誤ると、データの不整合やセキュリティリスクを招く可能性があるため、適切な設計と管理が重要である。

::: warning 本章の対象は業務アプリケーションとする
他章と同様に大規模toCサービスのようにキャッシュを最大限に活用できないという制約がある **業務アプリケーションを想定** して説明する。また、フロントエンドの主要なキャッシュに絞って解説する。
:::
...
| 手法                         | データ形式/構造                  | 永続性                 | 保存容量   | 操作   | 主な用途/適性                                              | 特徴/備考                                                                                                                                                                                                                                               |
| :--------------------------- | :------------------------------- | :--------------------- | :--------- | :----- | :--------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Web Storage (localStorage)   | シンプルなキー・バリュー         | ブラウザを閉じても保持 | 数MB程度   | 同期   | 少量の設定値、ユーザー情報など                             | 手軽だが容量制限あり。大容量データには不向き                                                                                                                                                                                                            |
| Web Storage (sessionStorage) |                                  | タブ閉じると消失       |            |        | 一時的な状態保持など                                       | localStorageより短命                                                                                                                                                                                                                                    |
| IndexedDB                    | オブジェクトストア, インデックス | 永続                   | 大容量可   | 非同期 | 大量データ、構造化データ、オフライン利用、インデックス検索 | 高機能だが操作がやや複雑                                                                                                                                                                                                                                |
...
本章では画像に関する各データフォーマットの特性比較に基づき用途に応じた選択基準を明確にする。さらに、実装における最適化やアクセシビリティ等のベストプラクティスを示し、開発における品質とパフォーマンスの両立を手助けする。
...
**参考**: [https://developer.mozilla.org/ja/docs/Web/Media/Guides/Formats/Image_types](https://developer.mozilla.org/ja/docs/Web/Media/Guides/Formats/Image_types#avif_%E7%94%BB%E5%83%8F)
...
**参考** [テストピラミッド万歳 | POSTD](https://postd.cc/long-live-test-pyramid/)
